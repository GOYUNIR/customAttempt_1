...
- **Cloudflare** — deploy with the official OpenNext adapter (`@opennextjs/cloudflare`); the repo ships the scaffolding (`open-next.config.ts` + root `wrangler.jsonc`) and a full walkthrough in `DEPLOY-CLOUDFLARE.md` (build with build-time env inlined, `wrangler deploy`, `wrangler secret put` for runtime secrets, `wrangler domains add` for a custom domain). The daily safety
