...
- **2026-09-07 — Hero shader reliability + unified controls + video fallback + layout controls (`hero-shader-reliability-video-layout`):**
  ...
  - **🧪 Verified:** `npx tsc --noEmit` clean (0 errors), `npm test` **397/397** (7 new `tests/shaders.test.ts` cases for text distribution / contrast scrim / render mode / clip normalization + selection), `eslint` 0 errors on every touched file (only the pre-existing BABEL 500KB note on `app/admin/page.tsx`). No new Redis keys.
