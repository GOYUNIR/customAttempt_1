...
- **Products** — add/edit/duplicate/publish/archive, price categories, Stripe
  ...
  **sampler (trial
  ...
  The variant editor is **mobile-first responsive**: cards wrap their labeled
  rows into a flex grid that collapses to `1fr/1fr` at `@media (max-width: 768px)`,
  every sellable input/select/button keeps a **44px touch target**, each Stripe
  Price ID input carries an inline **🔗 Auto-Inherited / ✏️ Custom Variant ID**
  badge, and bulky explainer banners are replaced with ⓘ hover tooltips.
...
| `STORAGE_PROVIDER` (optional) | Data backend selector — default (`supabase`) is the Supabase PostgREST store (`lib/storage/supabase.ts`, also the config + admin store). Set to `upstash` for Upstash REST Redis (battle-tested for payment/raffle writes) or `cloudflare-kv` for the Workers-KV adapter (`lib/storage/cloudflare-kv.ts`; read the concurrency caveats first). The active provider shows in `/admin → SetUp`. |
| `STORAGE_REPLICAS` (optional) | Comma-separated list of EXTRA stores that mirror every write (data-loss protection), e.g. `upstash` mirrors Supabase → Upstash. Read by `resolveReplicaProviders()` → `ReplicatedStorageClient` (`lib/storage/replicated.ts`): writes fan out to all mirrors, reads fail over to a mirror when the primary throws/returns empty. Set as a plaintext Variable (not a secret). |
| `SUPABASE_URL` / `SUPABASE_ANON_KEY` / `SUPABASE_SERVICE_ROLE_KEY` (optional) | The Setup Wizard's source of truth for `global_platform_settings` (provider keys + the platform configuration gate). Aliases: `NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_ANON_KEY`. **In production the Setup Wizard REQUIRES these as real environment variables** (the "lock on the vault" — the server needs them on every request to reach the database where the other keys are stored); inline wizard entry is dev-only. |
...
- **2026-09-08 — API Keys & Integrations 3×2 grid finalization + verification pass (`api-keys-grid-3x2-verification`):**
  - **🧱 The "API Keys & Integrations" panel is now a clean, symmetrical 3-column × 2-row grid** (`repeat(3, 1fr)` — was `repeat(auto-fit, minmax(260px, 1fr))`): **Row 1** = Payments | Transactional email | Address autofill (maps); **Row 2** = AI engine — prompt compiler (LLM) | 3D asset / image-to-3D engine | **Fallback AI engine**. The "Fallback AI engine" (secondary LLM provider + key) was extracted out of the "prompt compiler (LLM)" card into its own 6th block so every integration has an identical 3-field structure (provider dropdown / masked API key / model-or-endpoint input).
  - **✍️ Supabase access-token copy corrected.** The helper text no longer claims the token is "never stored in your database" — it now accurately states the token IS persisted (RLS-protected `operational_settings`, never echoed, never in any public payload) and reused by background health checks to auto-apply missing schema migrations on a cold start.
  - **✅ Full audit confirmed the 2026-09-08 master-fix is live and green**: provider-keys route normalizes all 18 camelCase/snake_case aliases and returns HTTP 200 envelopes (no bare 400/503); status/env-status/settings routes all have top-level try/catch + HTTP 200 fallbacks; `resolveSupabaseAccessToken`/`persistSupabaseAccessToken`/`autoApplySchema` persist + reuse the token; the async Tripo3D pipeline (`submitTask`/`pollTask` → `uploadHeroModel` → `hero-models/` Supabase bucket) is wired; WebGL 2.0 (`getContext('webgl2')`) + the GLSL 300 es sanitizer + the 32×32 `gridGeometry` shard mesh + `u_assemblyProgress`/`u_explodeRadius`/`u_twistIntensity` uniforms + the hardcoded `DEFAULT_VS`/`DEFAULT_FS` fallback shader are all present.
  - **🧪 Verified:** `npx tsc --noEmit` clean (0 errors), `npm test` **425/425**, `eslint app/admin/page.tsx` 0 errors/0 warnings (only the pre-existing BABEL 500KB note). No Redis keys added or changed.
- **2026-09-08 — Master fix: provider-keys 400 hardening + Supabase token persistence + async Tripo3D pipeline + ai3d_model (`master-fix-provider-keys-token-async-tripo3d`):**
  - **🔧 `/api/admin/provider-keys` no longer 400s on validation / casing.** The Setup Wizard posts camelCase field names (`supabaseAccessToken`) while the admin "API Keys & Integrations" panel posts snake_case (`supabase_access_token`) — both share this route. New `normalizeProviderAliases()` maps every known camelCase alias (`aiEngine`→`ai_provider`, `aiKey`→`ai_api_key`, `aiModel`→`ai_model`, `aiFallbackEngine`/`aiFallbackKey`→`ai_provider_secondary`/`ai_api_key_secondary`, `ai3dEngine`/`ai3dKey`/`ai3dEndpoint`/`ai3dModel`→`ai3d_provider`/`ai3d_key`/`ai3d_endpoint`/`ai3d_model`, `supabaseToken`/`supabaseAccessToken`→`supabase_access_token`) BEFORE validation, so a client can never be rejected for casing. Validation failures are logged verbosely (`console.error('[provider-keys] validation failed', …)`) and returned as an **HTTP 200** `{ ok:false, error, validationErrors }` envelope instead of a bare 400.
  - **🔑 Supabase access token is now PERSISTED (no longer discarded).** `supabase_access_token` added to `OperationalSettings` + `OPERATIONAL_SETTING_KEYS`; new `resolveSupabaseAccessToken()` (inline override → env → persisted `operational_settings.supabase_access_token`) and `persistSupabaseAccessToken()` (merges into the RLS-protected operational blob). The provider-keys route AND the setup wizard now persist the token, and `autoApplySchema()` resolves it asynchronously so the background schema self-heal can rebuild a wiped schema on a cold start without the operator re-entering the token. The token lives in `global_platform_settings.operational_settings` (service-role-only write, never returned by `toPublicSummary()`), so public client bundles never see it — the same RLS boundary already protects Stripe/Resend/AI/3D keys.
  - **⚡ Async Tripo3D pipeline + permanent asset caching.** `MeshDriver` gained optional `submitTask(imageUrl, prompt)` / `pollTask(taskId)` (returns a `MeshSubmitResult`); `Tripo3dDriver` now implements both and `generate()` is a thin submit→poll wrapper. `POST /api/ai/mesh` supports `mode: 'submit'` (returns `{ taskId, status:'processing' }` immediately — no Edge 10s timeout) and `mode: 'poll'` (resolves the task, downloads the GLB/GLTF on the server, and re-hosts it). New **`lib/supabase-storage.ts`** `uploadHeroModel()` downloads the model and uploads it to the public Supabase Storage bucket **`hero-models/`** (service-role key), returning the permanent `/storage/v1/object/public/…` CDN URL (falls back to the provider URL when Storage is unconfigured). The route returns both `modelUrl` and `storedUrl`.
  - **🎛 `ai3d_model` — dynamic 3D engine model customization (item 5).** New optional model selector for the image-to-3D engine (e.g. `tripo3d-v2.0`, `tripo3d-v2.5`, `meshy-4`), mirroring `ai_model` for the LLM. Wired end-to-end: `GlobalPlatformSettings`/`PlatformSettingsInput`/`PlatformSettingsPublicSummary`/`parseSettingsRow`/`toPublicSummary`, both normalizers + `savePlatformSettings`, the `probePlatformSettingsSchema` select list, the admin "3D asset / image-to-3D engine" card (model input), and the mesh drivers (Tripo3D forwards `model`, Meshy forwards `model_version`). New idempotent migration **`supabase/migrations/00007_ai3d_model.sql`** (+ column in `00001_init.sql`, `MIGRATION_00007` + fix-plan in `lib/setup-schema-guide.ts`, the `00007` row in `lib/supabase-migrate.ts`).
  - **🧪 Verified:** `npx tsc --noEmit` clean (0 errors), `npm test` **425/425** (2 new `tests/mesh-driver.test.ts` submit/poll cases + updated `tests/setup-schema-guide.test.ts` byte-for-byte/full-plan/ai-3d-mesh cases), `eslint` 0/0 on every touched file. No new Redis keys (the token lives in `operational_settings`; `ai3d_model` is a settings column; the model asset lives in the Supabase Storage `hero-models/` bucket).
- **2026-09-07 — Modular 3D mesh / image-to-3D AI providers (Tripo3D, Meshy, Stability 3D, Custom Webhook) alongside the LLM prompt compiler (`ai3d-mesh-providers`):**
  - **🧱 New 3D mesh driver layer (`services/ai/`).** `mesh-driver.ts` (the `MeshDriver` contract + `MeshGenerateResult` + `normalizeMeshFormat` + `sleep`), `tripo3d.ts` (`Tripo3dDriver` — POST `/v2/openapi/task` then polls until `success`, returns GLB/GLTF URL + thumbnail), `meshy.ts`, `stability3d.ts`, `custom-webhook.ts` (POST `{ imageUrl, prompt }` → model URL), `mesh-registry.ts` (`createMeshDriver` + `MESH_DRIVER_CATALOG`), and `mesh-factory.ts` (`MeshFactory.getDriver()` — platform settings → legacy env `TRIPO3D_API_KEY`/`MESHY_API_KEY`/`STABILITY3D_API_KEY`). All drivers are pure (zero `@/` imports, injectable `fetchImpl`) so `node --test` loads them directly.
  - **🔀 Image-to-3D routing endpoint (`app/api/ai/mesh/route.ts`).** Admin-gated + rate-limited POST that resolves the mesh driver and returns `{ ok: true, configured: false }` when no 3D provider is set (so the client degrades to the 2D shader WITHOUT throwing), or `{ modelUrl, thumbnailUrl, format }` on success — with a masked `meshError` on failure.
  - **🗄 Config serialization.** New `Ai3dProvider` type (`tripo3d`/`meshy`/`stability_3d`/`custom_webhook`), `AI3D_PROVIDERS`, and `sanitizeAi3dProvider()` (digits-allowed — `tripo3d`/`stability_3d` contain `3`); `GlobalPlatformSettings`/`PlatformSettingsInput`/`PlatformSettingsPublicSummary` gained `ai_model`, `ai3d_provider`, `ai3d_key` (write-only secret), `ai3d_endpoint`; `parseSettingsRow`/`toPublicSummary`/`normalizePlatformSettingsInput`/`normalizePlatformSettingsPatch`/`savePlatformSettings` all pass the new fields through. New idempotent migration **`supabase/migrations/00006_ai_3d_mesh.sql`** (+ columns in `00001_init.sql`, `MIGRATION_00006` + `ai_3d_mesh` fix-plan in `lib/setup-schema-guide.ts`, the `00006` row in `lib/supabase-migrate.ts`, and the `probePlatformSettingsSchema` select list).
  - **🎛 Setup panel / API Keys & Integrations.** The AI engine card is now **"prompt compiler (LLM)"** (provider dropdown + key + **model selector**) plus a new **"3D asset / image-to-3D engine"** card (Tripo3D/Meshy/Stability 3D/Custom Webhook dropdown + API key + base URL/endpoint). The bottom status bar reads **`AI: DeepSeek (LLM) + Tripo3D (3D Mesh)`**. Keys are write-only (cleared after save, never echoed).
  - **🖥 HeroShaderCanvas seamless fallback.** New optional `mesh3dUrl` prop: when set, a lazily-injected `<model-viewer>` web component renders the generated GLB/GLTF behind the hero content; when unset (no 3D key), the 2D image-texture WebGL shader renders as before — neither path throws.
  - **🧪 Verified:** `npx tsc --noEmit` clean (0 errors), `npm test` **423/423** (10 new `tests/mesh-driver.test.ts` cases + updated `tests/setup-schema-guide.test.ts` byte-for-byte/full-plan/fix-plan cases), `eslint` 0 errors on every touched file (only the pre-existing BABEL 500KB note on `app/admin/page.tsx`).
- **2026-09-07 — Fix Cloudflare 1102 + rich 3D shard mesh + seamless WebM loop + split/bottom layout (`cf1102-shard-mesh-webm-loop-layout`):**
  - **☁️ Cloudflare Error 1102 (resource exceeded) — clip blobs moved out of the Redis/Edge config.** New browser-only **`lib/shaders/clipStore.ts`** persists pre-rendered hero WebM/MP4 blobs in **IndexedDB** (`putHeroClipBlob` / `getHeroClipBlob` / `deleteHeroClipBlob`), so the multi-MB base64 video never round-trips through `/api/store` / `/api/catalog/status` / the Cloudflare Edge. `HeroClip` gained an optional `url` + `storedLocally` flag; `resolveHeroClips` accepts metadata-only clips (no `data:video/` URL required). The admin **`recordClip`** stores the blob in IndexedDB and persists only a tiny metadata record (id/mime/bytes/dims/duration); `app/page.tsx` resolves the `<video>` source from IndexedDB at render (falls back to live WebGL when absent) and a new `ClipThumb` component previews locally-stored clips. The `/api/ai/shader-prompt` route already returns lightweight JSON specs (<10KB) — unchanged.
  - **🧱 Rich 3D mesh deconstruction (beyond flat-2D rotation).** New pure **`lib/shaders/gridGeometry.ts`** (`buildGridGeometry(32)` → a 33×33 interleaved `[x,y,u,v]` vertex grid + `Uint16Array` indices). `IMAGE_VS`/`IMAGE_FS` (`lib/shaders/glsl.ts`) were rewritten: the vertex shader slices the product texture into **32×32 floating 3D shards**, twists them radially (`u_twistIntensity`), extrudes them by **image luminance** (a true relief), pushes them outward (`u_explodeRadius`), and projects through a simple perspective. Prompt intent now flows into uniforms `u_time`, `u_assemblyProgress`, `u_explodeRadius`, and `u_twistIntensity`; `HeroShaderCanvas` gained a `twistIntensity` prop (driven by `resolveHeroIntensity`) and renders via `drawElements` over the index buffer.
  - **🎬 Seamless WebM loop recording.** New `LOOP_PERIOD_SECONDS = 4.0` (canonical loop) in `presets.ts`; `computeAssemblyProgress`'s `pulse` mode is now periodic with exactly that period. `videoExport.ts` gained `heroLoopDurationMs(speed)` and `recordCanvasVideo` records exactly one loop period (`4.0s / speed`) and fires an `onStart` callback on the same tick capture begins. `HeroShaderCanvas` exposes an `onResetRef` timeline-reset handle (resets `u_time = 0`), and the admin `recordClip` wires it so the WebM's first and last frames share the same phase — the storefront mobile `<video>` loops with zero visible jump.
  - **📐 Split/Bottom layout unblocks the product view.** New pure helpers `isSplitHeroTextLayout()` and `heroTextJustify()` in `presets.ts`: both `split` **and** `bottom` now pin the brand/title block to the top edge and the CTA + raffle pill to the bottom edge via a flex spacer, keeping the center of the hero box clear for the 3D animation (the old `bottom` mode bundled title+CTA at the bottom). Wired into `app/page.tsx` and the admin `Live Viewport Preview` identically.
  - **🧪 Verified:** `npx tsc --noEmit` clean (0 errors), `npm test` **414/414** (8 new `tests/hero-mesh.test.ts` cases), `eslint` 0/0 on every touched file, `npm run build` compiles (`✓ Compiled successfully`). No new Redis keys — clips now live in browser IndexedDB; `twistIntensity` is a prop (not persisted).

- **2026-09-07 — Enforce WebGL 2.0 + GLSL 300 es sanitizer + guaranteed default shader (`webgl2-glsl300-sanitize-default`):**
  - **🎯 Eliminated the `'out' : syntax error` at the root.** `lib/shaders/glsl.ts` was converted from GLSL ES 1.00 (`attribute`/`varying`/`texture2D()`/`gl_FragColor`) to **pure GLSL ES 3.00** (`#version 300 es` on line 1, `in`/`out`, `texture()`, `out vec4 fragColor`) across all six shaders (FRAGMENT_VS/FS, IMAGE_VS/FS, PARTICLE_VS/FS). Mixing `#version 300 es` with WebGL1 syntax is exactly what produced `ERROR: 0:1: 'out' : syntax error`.
  - **🛡 Strict WebGL 2.0 context.** `components/HeroShaderCanvas.tsx` now requests ONLY `canvas.getContext('webgl2', { alpha: true, antialias: true, preserveDrawingBuffer: true, premultipliedAlpha: false, failIfMajorPerformanceCaveat: false })` — the WebGL1 `getContext('webgl')` fallback was removed, so GLSL 3.00 is never compiled against a WebGL1 context. `preserveDrawingBuffer: true` enables admin clip capture.
  - **🧹 GLSL sanitizer (`lib/shaders/glslSanitize.ts`).** New pure module `sanitizeGlslSource()` (plus `extractGlslFence` / `stripLeadingText` / `stripVersionDirective` / `prependVersionDirective`) that strips markdown fences + leading conversational text, removes any existing `#version` directive, drops a leading `precision` line, and prepends exactly `#version 300 es\nprecision highp float;\n`. Idempotent. The renderer sanitizes every compiled source, and `/api/ai/shader-prompt` now returns guaranteed-clean `vertexShader`/`fragmentShader` payloads built from `DEFAULT_VS`/`DEFAULT_FS`.
  - **🎨 Guaranteed default shader (never drops to CSS for a compile error).** `glsl.ts` gained `DEFAULT_VS`/`DEFAULT_FS` — a bulletproof 300 es textured quad that samples `u_productTexture` and applies a 3D float/spin. `HeroShaderCanvas` now uses a new `linkProgramWithFallback()` helper: on primary compile/link failure it recompiles the default shader (badge stays `WebGL 2.0 · N FPS`) instead of `showFallback('…')`; CSS is only reached when the context itself is unavailable or reduced-motion/low-power applies.
  - **🧪 Verified:** `npx tsc --noEmit` clean (0 errors), `npm test` **407/407** (7 new `tests/glslSanitize.test.ts` cases), `eslint` 0/0 on every touched file. No new Redis keys. (Layout UI parity + the 3-option Layout card / Advanced Metrics drawer were already shipped in the prior change.)

- **2026-09-07 — WebGL silent-fallback fix + database image texture CORS + admin↔storefront parity + layout UI cleanup (`webgl-image-cors-parity-layout`):**
  - **🖼 Database product-image textures now bind to WebGL reliably (`components/HeroShaderCanvas.tsx`).** `loadProductImage` was rewritten to load with `crossOrigin='anonymous'` first (CORS-clean → allowed to upload), and on failure retry WITHOUT the attribute for cross-origin CDN URLs that omit `Access-Control-Allow-Origin` — flagging the decoded bitmap `tainted` so the caller SKIPS the guaranteed-to-throw `texImage2D` upload instead of letting a `SecurityError` silently destroy the WebGL context. The texture is only uploaded AFTER the image fully decodes, and every failure path now logs an explicit `console.error/warn` (`isSameOriginUrl` helper added).
  - **🔬 Explicit GLSL diagnostics.** `compileShader` / `linkProgram` now log `getShaderInfoLog` / `getProgramInfoLog` on failure, and `showFallback(reason)` logs WHY the engine dropped to the CSS ambient gradient (WebGL context unavailable, shader compile/link error, reduced-motion/low-power) — no more silent fallback.
  - **🎯 Guaranteed WebGL shader (no CSS fallback for the exploded preset).** The `exploded_rebuild` preset now ALWAYS uses the fixed `IMAGE_VS`/`IMAGE_FS` textured quad (spin/assemble/disassembly deformation) with an in-shader `u_hasTexture=0` gradient fallback when no product image is available — the old procedural `PARTICLE_VS`/`PARTICLE_FS` point-cloud branch was removed (and its `buildProductGeometry`/`particleCount`/`productSilhouette` wiring deprecated on the component). A new `respectReducedMotion` prop lets the admin Live Viewport Preview force WebGL so the badge reads `WebGL 2.0 · N FPS` instead of `CSS Ambient Fallback · 0 FPS`.
  - **🧭 Exact admin↔storefront parity.** The admin preview's "Split View" text distribution now anchors the container to `flex-start` with a flex spacer (matching `app/page.tsx`) instead of the divergent `space-between`; both surfaces now place the title block at top and the CTA row at bottom identically.
  - **🧹 Layout UI cleanup (`components/admin/HeroShaderSettings.tsx`).** The Layout card now shows ONLY **Layout Preset**, **Text Distribution**, and **Contrast Scrim**; **Overlay Mode** and **Hero Box Height / Aspect Ratio** (incl. the custom-height slider) moved INSIDE the default-closed **⚙️ Advanced Metrics** drawer (alongside Max Width / Corner Radius / Padding).
  - **🧪 Verified:** `npx tsc --noEmit` clean (0 errors), `npm test` **400/400**, `eslint` 0/0 on every touched file. No new Redis keys.

- **2026-09-07 — Real product-image shader engine + layout presets + admin/backend hardening (`real-product-image-shader-layout-presets`):**
  - **🖼 Real product-image shader (fix pixelated generic dot-clouds).** `lib/shaders/glsl.ts` gained `IMAGE_VS`/`IMAGE_FS` — a full-screen quad shader with a `u_productTexture` (`sampler2D`) that samples the selected product's PRIMARY image and alpha mask directly, driving disassembly / assembly / spin / explosion on crisp product tiles (grid-cell offset + shrinkage opens seams during an explosion) instead of the procedural point-cloud. `components/HeroShaderCanvas.tsx` gained a `productImageUrl` prop: when the exploded-rebuild preset AND a product image are present it binds the image as a WebGL texture (`UNPACK_FLIP_Y_WEBGL` + `LINEAR` min filter + `CLAMP_TO_EDGE`, NPOT-safe), loaded asynchronously with a 1×1 placeholder so the sampler is always bound; any load failure (CORS/404/decode) silently falls back to the procedural particle engine. The image branch guards every `getAttribLocation` and binds the `a_position` VBO before `drawArrays`, and the async loader + cleanup track a single `textureToDispose` so textures are never double-deleted. `app/page.tsx` passes `productImageUrl={heroCoverImage}`; the admin preview passes `productImageUrl={previewImageUrl}`.
  - **📐 Layout presets (3 primary looks).** `lib/shaders/presets.ts` gained `HeroLayoutPreset` (`heroCard` / `fullBleed` / `splitBanner`), `LAYOUT_PRESET_OPTIONS`, and pure helpers `resolveHeroLayoutPreset` / `layoutPresetToContainerTarget` / `isFullBleedLayout`. `app/page.tsx` now derives the canvas placement + full-bleed styling (no border/radius/shadow, max-width 100%) from `layoutPreset` (falling back to the legacy `containerTarget` field). The admin Layout card was reordered: **Layout Preset**, **Text Distribution**, **Contrast Scrim** (front and center), **Overlay Mode**, **Hero Box Height**, and the fine-grained **Max Width / Corner Radius / Padding** sliders moved into a default-closed **⚙️ Advanced Metrics** drawer. `layoutPreset` is wired end-to-end (`goyunir.config.ts`, `lib/store-config.ts`, `lib/storefront-config.ts`, `components/ThemeProvider.tsx`) — no new Redis keys.
  - **🛡 Backend/WebGL hardening.** `/api/ai/shader-prompt/route.ts` now wraps the whole handler in try/catch and returns HTTP 200 `{ success: false, aiError: '…' }` fallback JSON (never an unhandled 503/500); success responses also carry `success: true`. `HeroShaderSettings.executePrompt` adds a `45% — Processing product image alpha channels` progress step. The admin Live Viewport Preview mirrors full-bleed styling.
  - **🧪 Verified:** `npx tsc --noEmit` clean (0 errors), `npm test` **400/400** (3 new `tests/shaders.test.ts` cases for the layout-preset helpers), `eslint` 0/0 on every touched file.

- **2026-09-07 — Hero shader reliability + unified controls + video fallback + layout controls (`hero-shader-reliability-video-layout`):**
  - **🔒 Backend 503 hardening (`app/api/ai/shader-prompt/route.ts`).** The AI prompt-compiler route can no longer 503 on transient failures: `authorized()` (which calls `isSuperAdminSession` → Redis) is wrapped in try/catch (denies safely), `getLicenseStatus()` is wrapped in try/catch (falls back to a permissive ACTIVE verdict — the compiler is non-destructive), and the AI `driver.complete()` call is raced against a 20s `withTimeout()` so a hung provider times out into the deterministic floor with a clear `aiError` instead of wedging the route.
  - **🖥 WebGL `no buffer is bound to enabled attribute` fixed (`components/HeroShaderCanvas.tsx`).** Root cause: when the effect re-ran on a preset/theme switch, the particle program's `a_normal`/`a_component` vertex arrays stayed ENABLED while their VBOs were deleted — so the next `drawArrays` threw `INVALID_OPERATION`. The teardown now calls a new `disableAllVertexAttribs(gl)` BEFORE disposing buffers, and every `getAttribLocation` result is guarded (`if (loc >= 0)`) so only linker-kept attributes are enabled and bound. The in-loop `draw()` is also wrapped in a try/catch that degrades to the CSS ambient fallback on a mid-draw GPU crash.
  - **🛡 React Error Boundary (`components/HeroShaderCanvas.tsx`).** The default export is now a class-based `HeroShaderErrorBoundary` wrapping the inner `HeroShaderCanvas` (named export), so a GPU/render crash during render/effect setup unmounts the canvas (hero card background shows) instead of white-screening the page. Async in-loop errors are handled by the internal try/catch (they can't reach a boundary).
  - **🎛 Unified Generate/Pause button + % progress (`components/admin/HeroShaderSettings.tsx`).** The standalone "Execute Prompt" and "Pause Timeline" buttons are merged into ONE button: `⚡ Generate AI Animation` (idle) → `⏸ Pause` / `▶ Resume` (generating, pausing the live canvas timeline) with an adjacent `0% → 100%` counter. Removed the `progressStep`/`STEP_LABELS` three-step trail; replaced with a granular `progress` + `progressLog` and an expandable **📋 Progress log** dropdown ("8% Resolving product geometry…", "35% Extracting product geometry", "70% Compiling GLSL shader uniforms…", "90% Applying GLSL render payload…", "100% Complete").
  - **🎬 Video render mode + mobile fallback (`lib/shaders/videoExport.ts` + `presets.ts`).** New browser-only `recordCanvasVideo()` records the WebGL canvas via `captureStream` + `MediaRecorder` into a lightweight ~3s WebM (VP9→VP8→MP4 fallback) data URL. `AiHeroSettings` gained `renderMode` (`live`/`video`) and `clips: HeroClip[]`, with pure helpers `resolveHeroRenderMode` / `resolveHeroClips` / `pickHeroClip` / `resolveEffectiveHeroRender` (mobile viewport or low-power GPU + a clip → auto-serve the loop). The storefront (`app/page.tsx`) renders a looping `<video>` behind the hero copy when `video` mode (or the auto fallback) applies. The admin panel gained a **Render Mode & Clips** card (Live WebGL vs Pre-rendered Video switch, "🎬 Record WebM Clip", clip library with preview + delete). `HeroShaderCanvas` exposes `onCanvasRef` for recording. No new Redis keys — clips live in `store:config.aiHero.clips` (kept small via a 3s / 2.5 Mbps / preview-resolution cap).
  - **📐 Text Distribution + Contrast Scrim (`presets.ts` + `app/page.tsx` + admin preview).** New `textDistribution` (`centered` default / `top` / `split` / `bottom`) and `contrastScrim` (0–100) fields wired end-to-end (`goyunir.config.ts`, `lib/store-config.ts`, `lib/storefront-config.ts`, `components/ThemeProvider.tsx`, `/api/store` + `/api/admin/settings` pass-through). The hero card maps distribution to `justifyContent` (`split` inserts a flex spacer between the title block and the buttons), and a theme-tinted `color-mix` scrim (`cardBackground` → `scrim%`) sits between the shader and the text (zIndex 5) so copy stays legible regardless of shader colors. Positions sync across the storefront hero and the admin Live Viewport Preview.
  - **🧪 Verified:** `npx tsc --noEmit` clean (0 errors), `npm test` **397/397** (6 new `tests/shaders.test.ts` cases for text distribution / contrast scrim / render mode / clip normalization + selection), `eslint` 0 errors on every touched file (only the pre-existing BABEL 500KB note on `app/admin/page.tsx`). No new Redis keys.
- **2026-09-07 — AI Hero Shader admin streamline + WebGL execution fix + accurate storefront Live Viewport Preview (`ai-hero-streamline-webgl-preview`):**
  - **🧹 Admin UI simplified (`components/admin/HeroShaderSettings.tsx`).** The four secondary accordions collapsed into two always-visible cards: **Motion** (Motion Type / Speed / Intensity) and **Layout** (Hero Box Height/Aspect Ratio, Max Width, Corner Radius, Padding, Overlay Mode). **Removed** the Silhouette Override dropdown (silhouette is now auto-derived from the selected product target via `buildProductTarget`), the **Engine Presets** section, and the **Theme & Palette** section (palette is always auto-synced from the live theme). Product Target remains the single primary selector, plus a new "Enable AI Hero Shader" toggle.
  - **🎬 Enhanced motion + layout controls.** New `AiHeroSettings` fields `speed` (0.5×–2×), `motionType` (`spin` / `assembly` / `hover`), `heroHeight` (`compact`/`standard`/`tall`/`custom`), `heroHeightPx`, `maxWidth`, `cornerRadius`, and `padding`. New pure helpers in `lib/shaders/presets.ts` (`resolveHeroSpeed`, `resolveHeroMotionType`, `motionTypeToPreset`, `motionTypeToLoop`, `resolveHeroHeightPx`, `MOTION_TYPE_OPTIONS`, `HERO_HEIGHT_OPTIONS`). `motionType` maps onto the engine preset + loop (spin → `exploded_rebuild` + `spin` loop; assembly → `exploded_rebuild` + `pulse`; hover → `cyber_mesh` + `mouse`), and the storefront (`app/page.tsx`) derives `preset`/`animationLoop`/`speed` from these fields (falling back to legacy stored values so an existing config keeps its look).
  - **🖥 Accurate public-hero Live Viewport Preview.** The right column now renders the real public hero container box — top badge/brand name, "by our hands. to your hands." title, subtitle, "Browse drops" + "Our Story" action buttons, and a "Total raffle entries" pill inside the rounded, theme-colored frame — with the live `HeroShaderCanvas` rendered **directly behind the text overlay** (canvas `zIndex: 0`, content `zIndex: 10`). The three-step progress trail (1/3 geometry, 2/3 shader compile, 3/3 canvas apply) is retained on the overlay.
  - **🔧 WebGL execution + layering fixed (`components/HeroShaderCanvas.tsx`).** `failIfMajorPerformanceCaveat` set `false` so software-rendered WebGL (SwiftShader / remote / VM) initializes instead of falling back to the CSS gradient; the low-power heuristic now only opts out on genuinely single-core devices (no longer consults `navigator.deviceMemory`, which falsely reports 2GB on many capable machines); and the background canvas wrapper now pins `zIndex: 0` while the storefront hero content uses `zIndex: 10`, so the AI-generated 3D product geometry paints behind the hero copy. Added a `spin` animation-loop mode (fully assembled + continuous rotation).
  - **🧬 Config wiring.** `speed`/`motionType`/`heroHeight`/`heroHeightPx`/`maxWidth`/`cornerRadius`/`padding` added to `AiHeroSettings` + defaults in `goyunir.config.ts`, `lib/store-config.ts`, `lib/storefront-config.ts`, and `components/ThemeProvider.tsx` (default preset flipped to `exploded_rebuild`, loop `spin`, motion `spin`). The hero section now applies configurable `minHeight`, `maxWidth`, `cornerRadius`, and `padding`, with theme-aware numeric fallbacks. No new Redis keys — all fields live inside the existing `store:config.aiHero` JSON.
  - **🧪 Verified:** `npx tsc --noEmit` clean (0 errors), `npm test` **391/391** (4 new `tests/shaders.test.ts` cases for the new pure helpers), `eslint` 0/0 on every touched file.
- **2026-09-06 — AI shader builder overhaul: real AI execution + 3-step progress + minimal accordions (`shader-builder-overhaul-minimal`):**
  - **⚡ AI pipeline verified + inline errors.** `app/api/ai/shader-prompt/route.ts` now returns a masked `aiError` (unusable response / provider failure / thrown error) alongside `source` so the admin can see WHY the model fell back to the deterministic floor — the demo-lock bypass (`driver?.configured` skips the license 403) is unchanged and still resolves DeepSeek/OpenAI/… keys first. `HeroShaderSettings.executePrompt` pipes the returned `params`/`silhouette` straight into `onChange` (functional updaters, no stale clobber) and renders a red `⚠` alert under the prompt box on any API failure instead of silently resetting.
  - **⏳ Three-step progress indicator.** `phase` → `progressStep: 0|1|2|3`; Execute now walks `⏳ 1/3 Resolving product geometry & silhouette…` → `⚡ 2/3 Compiling shader uniforms with AI…` → `🎨 3/3 Applying GLSL render payload to canvas…` under the button and on the live-preview overlay.
  - **🧹 Bloat stripped.** Deleted the five hardcoded magic-prompt chip buttons (`MAGIC_PROMPT_PILLS` + its `MagicPromptPill` type removed from `lib/shaders/promptParser.ts`); only the functional `⚡ AI Auto-Enhance Prompt` remains. Removed the product-silhouette sub-caption, the `Derived: …` readout, every accordion `hint`, and the `0% exploded → 100% assembled.` caption.
  - **🎛 Accordions simplified to power controls.** **Engine Presets** = single dropdown; **Motion** = one `Intensity & Speed` slider + `Animation Mode` dropdown (Infinite Loop / On Scroll Scrub / Mouse Interactive); **Layout** = `Canvas Mode` toggle (Full Hero Background / Sub-Text Banner); **Theme & Palette** = sync toggle + swatch row only.
  - **🧬 New `intensity` knob.** `AiHeroSettings.intensity` (0–1) drives both dispersion and animation speed via `resolveHeroIntensity` / `intensityToRadius` / `intensityToSpeed` (pure helpers in `lib/shaders/presets.ts`, legacy `explosionRadius` fallback preserved). `HeroShaderCanvas` gained a `speed` prop (0.5×–2× timeline scale via a ref — no GL teardown) and `app/page.tsx` forwards it. Wired through `store-config.ts` / `storefront-config.ts` / `goyunir.config.ts` / `ThemeProvider.tsx` types+defaults. No new Redis keys.
  - **🧪 Verified:** `npx tsc --noEmit` clean (0 errors), `npm test` **387/387** (added intensity/speed helper coverage, removed the `MAGIC_PROMPT_PILLS` case), `eslint` 0/0 on all touched files.
- **2026-09-06 — HeroShaderSettings.tsx cleanup: core workflow first, crisp copy, two-phase generation states (`heroshader-settings-cleanup`):**
  - **🧭 Layout reordered.** The left column now leads with the **Smart AI Prompt Compiler** (Product Target → Silhouette override → Prompt → magic chips + Auto-Enhance → Execute/Pause/Cancel → compact derived readout), and the four secondary groups — **Engine Presets** (now collapsed by default), **Motion & Primitive Parameters**, **Placement & Canvas Layout**, and **Theme & Palette** — sit below as collapsible accordions.
  - **✍️ Microcopy trimmed.** Removed the verbose `Canvas behind the whole hero card…` / `Extracted live from the active theme accent colors…` / `0% exploded → 100% assembled · drives the timeline on Manual Scrub` captions and the viscosity/turbulence readout; the parent `app/admin/page.tsx` "A luxury multi-mode GPU shader…" paragraph is now a one-liner.
  - **⚙️ Two-phase generation feedback.** `generating` boolean → `phase: 'idle' | 'compiling' | 'generating'`; Execute now shows "⏳ Compiling shader…" (deterministic floor) then "⏳ Generating preview…" (AI route) on both the button and the live-preview overlay, and errors surface inline.
  - **✅ Product Target stays fully dynamic.** No hardcoded 'test1'/'testSample1'/mock entries — the dropdown maps the live `/api/admin/products` catalog via the `products` prop; the pipeline already feeds prompt + selected product target to `/api/ai/shader-prompt`.
  - **🧪 Verified:** `npx tsc --noEmit` clean (0 errors), `npm test` **384/384**, `eslint components/admin/HeroShaderSettings.tsx` 0/0. No Redis keys changed.

- **2026-09-06 — AI Hero shader suite fix + UX cleanup: license bypass, color-input sanitize, 503 hardening, accordion redesign (`shader-suite-fix-ux-cleanup`):**
  - **🔓 Demo Mode license bypass (`app/api/ai/shader-prompt/route.ts`).** The prompt-compiler route now resolves the AI driver FIRST and skips the `Demo Mode: AI generation is disabled…` 403 whenever a real API key is configured (`driver?.configured` — DeepSeek/OpenAI/… in settings or env, or a Workers AI binding). An operator who wired a paid key is no longer locked out of the compiler; the license block only applies when NO AI provider is configured. The driver fetch + `driver.complete()` are both wrapped so a transient Supabase read / AI provider failure falls back to the deterministic floor instead of 503ing the route.
  - **🎨 Color-input sanitization (`components/admin/HeroShaderSettings.tsx`).** The Theme & Palette swatches fed `paletteToCss()` output (`rgb(r, g, b)`) straight into `<input type="color" value=…>`, which Chrome logs as a format warning on every render. They now bind `toHexColor(col)` (existing helper in `lib/share-card-config.ts`) so the value is always `#rrggbb`.
  - **🛡 Graceful 503 handling on admin status/setting routes.** `app/api/admin/env-status/route.ts` was rewritten with a full try-catch that falls back to the pure `discoverEnvironment()` registry (never secrets, never throws) when Supabase/Redis is down or the device check throws — the SetUp tab stays usable instead of cascading 503s. `app/api/admin/status/route.ts` and `app/api/admin/self-test/route.ts` now guard the `adminAuthorized()` call (which can throw via `isSuperAdminSession` → Redis) and, in self-test, the platform-settings resolution, the `store:config` read, and `loadProducts(redis)` each fail soft with a per-check result instead of an unhandled throw.
  - **🧭 Accordion redesign of the AI Hero panel.** The left column is reorganized so ONLY the **Smart AI Prompt Compiler** (Product Target dropdown + silhouette override + prompt + Execute/Pause/Cancel + derived readout — the Product Target card was merged in) sits side-by-side with the sticky Live Viewport Preview. The four secondary groups — **Engine Presets**, **Motion & Primitive Parameters**, **Placement & Canvas Layout**, and **Theme & Palette** — are now collapsible accordions (new local `Section` component with a 1-line hint caption when collapsed). Long paragraph descriptions were trimmed to one-line captions (`No product — neutral container.`, `Canvas behind the whole hero card…`, `0% exploded → 100% assembled…`).
  - **🧪 Verified:** `npx tsc --noEmit` clean (0 errors), `npm test` **384/384**, `npx eslint .` **0 errors/0 warnings** (only the pre-existing BABEL 500KB note on `app/admin/page.tsx`). WebGL disposal re-verified: `HeroShaderCanvas`'s `dispose` array (`deleteProgram`/`deleteShader`/`deleteBuffer`) runs in the effect cleanup, which re-fires on preset/mode/silhouette switch (all are in the dependency array). No new Redis keys. The `take control.` CTA in `app/page.tsx` was already a non-interactive `<span>` (prior change) — no further edit needed.

- **2026-09-06 — Home hero "take control." CTA renders as subtle inline text (`hero-story-body-inline-text`):**
  - **✍️ `app/page.tsx`** — the default CTA slot labeled `take control.` (shown right of the "Our Story" pill) no longer renders as an interactive `<Link>` carrying padding. It is now a plain `<span>` with muted inline styling (`color: cardTextMuted`, `fontWeight 500`, `fontSize 12`, `letterSpacing 0.2px`) — no button/pill container, border, or background. It remains purely decorative side-text; the "Our Story" pill and the primary "Browse drops" CTA are unchanged.
  - **🧪 Verified:** `npx tsc --noEmit` clean, `eslint app/page.tsx` 0 errors. No Redis keys changed.

- **2026-09-06 — AI Hero Banner & Shader refactor: dynamic product targeting + live execution controls + memory hygiene (`ai-hero-shader-dynamic-target-live-controls`):**
  - **🎯 Dynamic product target selection.** New pure module `lib/shaders/productTarget.ts` (`SilhouetteKey`, `SILHOUETTE_KEYS`, `normalizeSilhouette`, `resolveProductSilhouette`, `buildProductTarget`, `silhouetteLabel`) maps a LIVE catalog item's own metadata (name/slug/categories) onto a bounded generic container key (bottle/jar/box/card/tube/generic) with strict word-boundary matching — a brand name that merely CONTAINS a container word (e.g. "Boxwood") stays `generic`, so no brand/product name is ever hardcoded. The admin AI Hero panel gained a **"Product Target" dropdown** (tied to `/api/admin/products` via a new `products` prop) plus a manual silhouette override, and the selected product's metadata/silhouette flows into the prompt-compiler payload.
  - **⚙️ Generation lifecycle & execution controls.** `components/admin/HeroShaderSettings.tsx` now has an async **Execute/Generate** (deterministic `compileShaderParams` first for instant preview, then a new admin AI endpoint to refine), **⏸ Pause / ▶ Resume Timeline** (freezes/resumes the canvas via a `paused` prop + ref — no GL context teardown), and **✕ Cancel** (AbortController aborts the in-flight request and restores the pre-Execute snapshot). A loading overlay ("Compiling shader… — keeping the ambient fallback active") covers the live viewport during generation.
  - **🧠 Prompt compiler + AI endpoint.** `lib/shaders/promptParser.ts` gained `compileShaderParams` (injects the target's silhouette), `buildShaderPrompt` (structured payload with product metadata) and `parseShaderParamsResult` (bounded, clamp + enum-validated JSON parse — a hallucinated key can never reach the GPU). New admin-only route **`app/api/ai/shader-prompt`** (license-gated + rate-limited + usage-tracked) compiles the prompt deterministically and, when an AI provider is configured, merges the model's refinement over the deterministic floor.
  - **🧬 Dynamic procedural geometry.** `lib/shaders/bottleGeometry.ts` gained `buildProductGeometry(silhouette, pointCount, seed)` — the exploded point cloud now derives its shape from the silhouette key (jar/box/card/tube have their own builders; bottle/unknown fall back to the neutral bottle), replacing the hardcoded "always a bottle" geometry. `HeroShaderCanvas` accepts `productSilhouette` + `paused` and wires both through.
  - **🧬 Config + wiring.** `AiHeroSettings` gained `targetProductId` / `targetProductName` / `productSilhouette` (wired through `goyunir.config.ts`, `lib/store-config.ts`, `lib/storefront-config.ts`, `components/ThemeProvider.tsx`, `/api/store` merge, and `/api/admin/settings` pass-through). `app/page.tsx` forwards `productSilhouette` to the storefront `HeroShaderCanvas` so admin target changes persist to Redis and sync to the home hero. `app/admin/page.tsx` merges `next.aiHero` over `defaultAiHeroSettings()` on hydration so legacy configs re-hydrate the new surface.
  - **🛡 Memory hygiene.** The existing `dispose` array (deleteProgram/deleteShader/deleteBuffer) remains the explicit `.dispose()` path, run on unmount AND on preset/mode/silhouette switch (effect re-run); the renderer has no texture units, so no texture disposal is needed. `paused` toggling is ref-synced via `useEffect` (never a ref write during render — fixes the `react-hooks/refs` lint rule).
  - **🧪 Verified:** `npx tsc --noEmit` clean (0 errors), `npm test` **384/384** (13 new `tests/shader-target.test.ts` cases: silhouette resolution, payload building, prompt compiler, AI-param parse/clamp, dynamic geometry), `eslint` 0 errors on every touched file (`app/admin/page.tsx` emits only the known BABEL 500KB note), `npm run build` compiles (`/api/ai/shader-prompt` included). No new Redis keys — `targetProductId`/`targetProductName`/`productSilhouette` live inside the existing `store:config.aiHero` JSON.

- **2026-09-06 — Zero-hardcoding shader/UX finalization: fixed `presets.ts` duplicate-declaration build break + reconciled shader tests with the generic prompt interpreter (`zero-hardcoding-finalize-shader-ux`):**
  - **🐛 `npx tsc --noEmit` restored to 0 errors.** `lib/shaders/presets.ts` carried a half-merged duplicate block — `PARTICLE_COUNT_OPTIONS` / `EXPLOSION_RADIUS_MIN` / `EXPLOSION_RADIUS_MAX` were declared twice and `defaultAiHeroSettings()` was defined twice (the second copy missing the new `containerTarget`/`canvasHeight`/`blendMode` fields), producing 11 TS errors (`TS2451`/`TS2323`/`TS2393`/`TS2739`). The trailing duplicate block was removed, leaving the single canonical `defaultAiHeroSettings()` (with `containerTarget: 'background'`, `canvasHeight: 'medium'`, `blendMode: 'normal'`).
  - **🧪 `tests/shaders.test.ts` reconciled with the zero-hardcoding prompt interpreter.** Two cases still asserted the OLD hardcoded-brand behavior the parser deliberately removed: `perfume` → `productSilhouette === 'perfume'` (now bound to the generic `bottle` silhouette key) and a hardcoded `roccstar` brand token → `bottle`. Both were rewritten to assert the generic container-descriptor mapping (`perfume` → `bottle`; a `bottle` descriptor → `bottle`), so no brand/product name ever appears in a test assertion.
  - **🔍 Full six-section audit confirmed already-shipped + consistent:** the 2-column admin shader suite (`components/admin/HeroShaderSettings.tsx`) with the "Execute Prompt & Generate Preview" action, magic prompt pills, container/canvas/blend controls + sticky live viewport (FPS meter / render badge / device toggle / assembly scrubber); the multi-primitive `components/HeroShaderCanvas.tsx` + `lib/shaders/*` engine (prompt compiler, GLSL raymarch/domain-warp, procedural bottle particle assembly, `.dispose()` hygiene, `webglcontextlost` fallback, SSR guards); homepage hero canvas layering (`background` vs inline `banner` behind `zIndex: 1` copy, with the `take control.` line as subtle side-text — never a forced pill); PDP variant slug/price binding (`variantSizeLabel`, `humanizeBadge`, `PLACEHOLDER_SIZE_RE`) + `sanitizeNotes()` editorial scrub (drops `notesnstuff`/`sanple`/`test`/`todo`/`lorem`/blank bodies); and the mobile scroll cue ("Keep scrolling ↓", auto-fade past 50px, `:has()` suppression under the sticky CTA) + CTA dedup + `.goyunir-field` ellipsis polish.
  - **🧪 Verified:** `npx tsc --noEmit` clean (0 errors), `npm test` **371/371**, `eslint` 0 errors on every touched file. No new Redis keys.

- **2026-09-06 — Master engine & admin UI overhaul: advanced hero shader, 3D exploded product assembly, prompt interpreter & production guardrails (`hero-shader-master-engine-admin-ui`):**
  - **🎛 New admin control suite `components/admin/HeroShaderSettings.tsx`.** The old single-column AI Hero panel is now a **luxury 2-column grid** (`grid grid-cols-1 lg:grid-cols-12 gap-8`): left column (7 cols) holds **Mode & Presets** (visual preset cards with active ring state + category badges), **AI Prompt & Smart Parameters** (magic tag pills + "⚡ AI Auto-Enhance Prompt" + a live derived-params readout), **Motion & Explosion Controls** (explosion-radius 0–150mm slider, particle-count 10k/50k/100k switcher, opacity slider with 25/50/75/100% quick buttons, depth-blur slider, animation-loop selector), and **Theme & Palette** (accent-vector sync toggle + three swatches); right column (5 cols) is a **sticky live viewport preview** with a real WebGL canvas, a live FPS meter + render-status badge (`WebGL 2.0 · N FPS` / `CSS Ambient Fallback`), Desktop/Mobile viewport toggle, and an **interactive assembly scrubber** (0% exploded → 100% assembled).
  - **🧠 Smart AI prompt interpreter `lib/shaders/promptParser.ts`** (`parsePromptToParams`, `enhancePrompt`, `MAGIC_PROMPT_PILLS`): maps natural language onto bounded render uniforms — "exploded/rebuild/assemble" → exploded mode + assembly timeline + radial dispersion; "fluid/organic/smoke" → viscosity/warp-frequency/turbulence; "liquid glass/refractive" → raymarched glass; "roccstar"/product names → bottle silhouette binding; "particle/cosmic/mesh" → particle mode.
  - **⚙️ Multi-engine renderer `components/HeroShaderCanvas.tsx`** (replaces `HeroAnimationCanvas.tsx`): **Mode A** GLSL fragment shader (domain-warped organic, cursor-reactive cyber mesh, raymarched refractive glass) and **Mode B** a 3D particle point-cloud that **reassembles a procedurally generated perfume bottle** (cap / nozzle / vessel / label) along surface normals via `u_assemblyProgress`, with pulse / scroll-scrub / mouse-distance loop modes. GLSL sources live in `lib/shaders/glsl.ts`; geometry in `lib/shaders/bottleGeometry.ts` (deterministic mulberry32); theme→accent-vector extraction in `lib/shaders/palette.ts`.
  - **🛡 Production guardrails.** All GL resources (buffers/programs/shaders) are deleted on unmount + recreated on preset/mode switch; WebGL is gated behind `failIfMajorPerformanceCaveat`, `prefers-reduced-motion` and a low-power heuristic (≤2 cores / ≤2GB) with a pure-CSS ambient-gradient fallback; **`webglcontextlost`** degrades to the CSS gradient and `restored` re-shows the canvas; SSR is safe (client-only `'use client'` + `typeof window` guards); the bottle is **procedurally generated** so there is no model file to fail.
  - **🧬 Config + wiring.** `store:config.aiHero` gained `explosionRadius`, `particleCount`, `depthBlur`, `animationLoop`, `assemblyProgress`, `paletteAutoSync`, `accentA/B/C` — wired end-to-end (`goyunir.config.ts`, `lib/store-config.ts` type + defaults, `lib/storefront-config.ts` type + default, `components/ThemeProvider.tsx` `LiveThemeValue`, `app/page.tsx`, `/api/store` merge, `/api/admin/settings` pass-through, and `app/admin/page.tsx`). Legacy preset ids (`ambient_mesh`/`particle_waves`) map onto the new canonical suite (`ambient_glass`/`cyber_mesh`) so an already-seeded store keeps rendering without a migration. **No new Redis keys** — everything lives inside the existing `store:config.aiHero` JSON.
  - **🧪 Verified:** `npx tsc --noEmit` clean, `npm test` **371/371** (18 new `tests/shaders.test.ts` cases), `eslint` 0 errors on all touched files (admin page.tsx emits only the known BABEL 500KB note), `next build` compiles (only the pre-existing "middleware file convention is deprecated" notice). No new Redis keys.
- **2026-09-06 — Executive mobile overhaul: CTA/z-index fixes, mobile action dedup, banner streamlining, size-chip & editorial sanitization, image shimmer fallbacks, footer links (`executive-mobile-overhaul`):**
  - **📌 Sticky CTA & overlay z-index (`components/Storefront.tsx`, `components/SiteChrome.tsx`, `app/globals.css`).** The global "Keep scrolling" cue (SiteChrome) is now a CSS class (`.goyunir-scroll-cue`, z-index 30) and is **completely suppressed** on a product page whenever the mobile sticky CTA bar is active via `body:has(main[data-goyunir-pdp-cta]) .goyunir-scroll-cue { display: none !important }` — the product `<main>` carries `data-goyunir-pdp-cta` only when `!soldOut && primaryCtaLabel`. `.goyunir-pdp-cta-bar` z-index raised 40 → **50** with `backdrop-filter: blur(16px)` + safe-area `env(safe-area-inset-bottom)` (already present).
  - **📱 Mobile action deduplication.** The in-panel "Enter allocation" / "Secure piece" buttons now carry `.goyunir-pdp-inline-primary` (hidden `<768px`) and "Add to bag" carries `.goyunir-pdp-add-bag` (full-width on mobile) so the sticky bar owns the primary action with no duplicate buttons.
  - **🏷 Banner streamlining.** The three stacked notification cards (urgency/status, "Earn X points", mixed-format ribbon) collapsed into ONE horizontal micro-badge row directly under the title (`.goyunir-pdp` — urgency text + ⭐ pts pill + 🎟 mixed pill + muted status line), reducing vertical height so size chips + checkout fields sit above the fold. `mixedBadgeLabel` still resolves the admin `mixedFormatRibbon` `{raffle}`/`{fcfs}` tokens.
  - **🧬 Size chip & variant label refactor.** `variantSizeLabel` no longer appends a redundant `- RAFFLE` suffix (the chip's raffle/buy badge pill conveys mode) and placeholder size strings (`yes`, `n/a`, `none`, …) fall back to a structured `Option N` label via `PLACEHOLDER_SIZE_RE`. Chips use `gap: 8px` + `flex-wrap: wrap` with `whiteSpace: nowrap` (no awkward internal text wraps).
  - **📝 Editorial sanitization.** New `sanitizeNotes()` filters out developer/draft placeholders (`notesnstuff`/`sanple`/`awesomeness`/`lorem`/`todo`/…) and blank bodies; "Why this drop matters" renders ONLY when real notes survive, wrapped in a refined serif card (`Georgia`).
  - **⌨️ Form input polish.** New `.goyunir-field` CSS utility (`text-overflow: ellipsis` placeholders, 16px font on `<768px` to prevent iOS auto-zoom) applied to the storefront email/address inputs + the drawer address input, with wider horizontal padding.
  - **🖼 Image shimmer fallbacks.** New `zeroImageStyle()` in `lib/media.ts` (brand-tinted `color-mix` gradient + the existing `goyunirZeroImage` keyframe) replaces every pitch-black `#1a1a1a` placeholder on the home product cards, catalog tiles + detail modal, and the live "Currently Available" tiles now render a **64×64 image thumbnail** (or shimmer) alongside the text. The raw `NO IMAGE` string is gone. `humanizeBadge()` turns the shared-pool `🔗` slug into a human label (`testsample1` → `Testsample 1`); the hero `take control.` line is now a secondary pill `<Link>`.
  - **🔗 Footer links.** Mobile footer legal links (Terms/Privacy/Shipping/Manage My Entry) wrapped with `gap: 12px`, row wrapping + `·` separators (attached to each link so they never orphan).
  - **🧪 Verified:** `npx tsc --noEmit` clean, `npm test` **353/353**, `eslint` 0 errors on every touched file. No new Redis keys.

- **2026-09-06 — Social-first storefront 2-column redesign + AI hero shader canvas + verification pass (`social-pdp-two-col-ai-hero-shader`):**
  - **🛍 Social-first storefront & 2-column product page (`components/Storefront.tsx` + `app/globals.css`).** The product page is now a responsive grid: mobile stays a single column (media → purchase panel → notes/cart), and on **≥768px the Media Gallery sits left with a sticky Purchase Panel (title → size chips → sample incentive → checkout fields → CTA) on the right** (`.goyunir-pdp-grid` / `.goyunir-pdp-sticky` / `.goyunir-pdp-full`). The page shell uses **`100dvh` + `calc(… + env(safe-area-inset-bottom))`** so iOS Safari + Instagram/TikTok in-app browser chrome never clips content, and a new **mobile sticky bottom checkout bar** (`.goyunir-pdp-cta-bar`) pins the primary action ("Enter allocation" / "Secure piece · $X") above the home indicator (hidden on desktop). The primary CTA is now derived once (`primaryCtaLabel` / `handlePrimaryCta` / `primaryCtaStyle`) so the in-panel button and the sticky bar can never drift — zero hardcoded product copy.
  - **🖼 Zero-image gallery fallback.** When a product has 0 media items, the gallery box renders a slow ambient shimmer/gradient placeholder (branded with the product name) instead of a blank black box — new `goyunirZeroImage` keyframe.
  - **🎛 AI Hero Banner & Shader Animation (Goal 5).** New `components/HeroAnimationCanvas.tsx` — a GPU-accelerated full-screen fragment-shader canvas (three generic procedural presets: **ambient mesh / particle waves / dark organic**, colors from live theme accents) with **WebGL feature detection (`failIfMajorPerformanceCaveat`), `prefers-reduced-motion`, a low-power heuristic (≤2 cores / ≤2GB deviceMemory) and a tab-hidden pause**, all falling back to a pure-CSS ambient gradient. Wired into `app/page.tsx` behind the hero card, with a new **admin → Settings → AI Hero** panel (`settings-aihero`: enable/disable, preset selector, opacity slider, AI prompt hint + a live canvas preview). Config shape `store:config.aiHero { enabled, preset, prompt, opacity }` added end-to-end (`goyunir.config.ts`, `lib/store-config.ts`, `lib/storefront-config.ts`, `components/ThemeProvider.tsx` `LiveThemeValue`, `app/layout.tsx` `buildLiveTheme`, `/api/store` `mergePublicConfig`, `/api/admin/settings`, and `app/admin/page.tsx` fetch/save). **No new Redis keys** — `aiHero` lives inside the existing `store:config` JSON.
  - **✅ Verification of already-shipped items.** Confirmed Goal 1 (Cloudflare 1102 shared-pool memoization via `indexSharedPools`/`findLiveInventoryForProduct` + the `/icon` favicon in `global-error.tsx`/`error.tsx`/layout metadata) and Goal 4 (synced-sample incentives via `categorySamplerPresentation`) and the admin `beforeunload` unsaved-changes guard are all present and correct; the "=== KEEP SCROLLING ===" debug string does not exist anywhere in the tree (only the legitimate "Keep scrolling" UI hint in `SiteChrome`).
  - **🧪 Verified:** `npx tsc --noEmit` clean, `npm test` **353/353**, `eslint` 0 errors on all touched files (`app/admin/page.tsx` emits only the known BABEL "file exceeds 500KB" note). No new Redis keys.

- **2026-09-06 — Mobile-first admin product editor overhaul + Stripe ID inheritance indicator (`mobile-first-variant-editor-stripe-inheritance-indicator`):**
  - **📱 Mobile/tablet responsive variant editor (`app/admin/page.tsx`, TAB 2 — Variants & Shared Inventory).** Each variant card was rebuilt on a **wrapping flex grid that collapses to a `1fr/1fr` grid at `@media (max-width: 768px)`** (long fields — Commerce Mode / Stripe Price ID — and the reorder handle span the full row). Layout lives in a self-contained `<style>` string (`variantEditorCss`, rendered once per tab) rather than inline `display`/`flexWrap`, because inline styles always beat stylesheet rules — including inside media queries — so the responsive stacking can never be overridden. No horizontal scrollbars/clipping.
  - **🧹 Removed the redundant column-header strip** ("REORDER VARIANT SKU PRICE UNITS STRIPE ID MODE ACTIONS") that misaligned on small screens. Each variant card now groups into **clean, labeled rows with persistent uppercase `<label>` tags** above every input — **SIZE NAME, SKU, PRICE ($), UNITS, STRIPE PRICE ID, COMMERCE MODE, MAX / EMAIL, MAX / CART, RAFFLE CAP, SYNC SLUG, WINNER TIERS** (new shared `variantInputStyle` / `touchButtonStyle` constants).
  - **👆 44px touch targets everywhere.** Every sellable input/select/button (incl. the ▲/▼ reorder handles, Auto SKU, Link/Unlink, ✕ remove, 🧪 sampler, + Add Variant) keeps a `min-height: 44px` via `variantInputStyle`/`touchButtonStyle` + a scoped CSS rule `[data-variant-row] input:not([type=checkbox]):not([type=radio]), [data-variant-row] select { min-height: 44px; }` so the sub-panels (commerce/sampler/raffle inputs) inherit it too. The Auto SKU button now sits cleanly beside the SKU input instead of compressing text.
  - **🔗 Stripe Price ID inheritance indicator.** The Stripe Price ID field header now shows a dynamic badge: **"🔗 Auto-Inherited"** (blue) when the variant's `stripeId` is blank / the `price_placeholder_not_configured` sentinel / the resolved store default (`defaultStripePriceId` state), vs **"✏️ Custom Variant ID"** (amber) when a real variant-specific `price_…` ID is set — visually distinguishing auto-prefilled IDs from manually customized ones (`.pfv-stripe-badge--inherited` / `--custom`).
  - **👁 Live preview sync audited — already correct, unchanged.** `ProductLivePreview` receives `product={productForm}` (live React state) and is NOT memoized, so every keystroke / mode switch / price adjustment / sampler toggle (all via functional `setProductForm`) re-renders the drawer in real time; the `key={editingProduct || 'new-product'}` remount only fires on product switch.
  - **🔗 Synced-card polish + banner-noise reduction.** The "🔗 Synced with [slug]" card now uses the standard `#0d0d10`/`#232329` card chrome (no more off-tone blue tint) with touch-friendly reorder buttons, and its "Inherits price/stock/SKU/Stripe…" explainer collapsed into an ⓘ tooltip. The bulky "⚡ This size sells instantly…" and "💡 If STRIPE_PRODUCT_ID is set…" banners were replaced with the new `InfoIcon` (ⓘ) hover tooltip component (pure CSS tooltip, no JS portal).
  - **🧪 Verified:** `npx tsc --noEmit` clean, `npm test` **353/353**, `eslint app/admin/page.tsx` exit 0 (only the known BABEL 500KB note). No new Redis keys; the indicator reads the existing `stripeId` field + the existing `defaultStripePriceId` client state.
- **2026-09-06 — Cloudflare 1102 SSR CPU memoization + favicon leak + synced-variant reordering + admin labels + synced sample incentives (`cf-1102-favicon-sync-reorder-labels-synced-sample-incentives`):**
  - **⚡ Cloudflare Error 1102 (per-request CPU/RAM) — shared-pool resolution memoized.** `lib/server-config.ts` gained `indexSharedPools(liveStates)` (builds `Map<slug, {inventoryRemaining, totalInventory}>` ONCE per request) and `findLiveInventoryForProduct(..., sharedPools?)` now takes that optional index instead of re-scanning the ENTIRE `liveStates` array for every product — turning the old O(products × liveStates) fold into O(products) Map lookups. `/api/store` and `/api/catalog/status` build the index once and pass it into the per-product `.map()`. No key changes; behavior identical.
  - **🖼 Vercel favicon leak fixed.** Deleted the shipped default `app/favicon.ico` (the ~25KB Vercel/Next triangle that leaked as the favicon on every fallback/error state) and added **`app/global-error.tsx`** (a full `<html>` document with an explicit `<link rel="icon" href="/icon">` + `apple-touch-icon`, since global-error renders OUTSIDE the root layout and has no metadata) and **`app/error.tsx`** (renders within the layout, inheriting the `/icon` favicon). The layout `generateMetadata` already advertises `icons: { icon: '/icon', apple: '/icon' }`. The dynamic `/icon` route keeps rendering the brand favicon/letter in every state — the default Vercel logo can never appear again.
  - **🔃 Synced variants are now reorderable.** The variant `▲/▼` reorder controls previously lived ONLY inside the `{!isSyncEnabled && …}` input grid, so a slug-synced variant (which unmounts that grid and shows the "🔗 Synced with [slug]" badge card) had no way to move up/down. The badge card now carries its own `▲/▼` buttons wired to the existing `movePriceCategory(idx, ±1)` (which already re-indexes `position` cleanly for every row regardless of sync state).
  - **🏷 Persistent uppercase field labels + helper text in the variant editor.** `app/admin/page.tsx` replaced the placeholder-only variant inputs with explicit, persistent `<label>` tags above every field — **Size Name, SKU, Price ($), Units, Stripe Price ID, Commerce Mode, Sync Slug, Winner Tiers** — via new shared `variantFieldLabelStyle` / `variantFieldWrapStyle` constants. Placeholders are now short neutral hints, complex inputs carry `title` tooltips, and the identity/SKU/price/units/Stripe row + sync prompt + commerce-mode row were re-aligned to `align-items: flex-end` for a cohesive card layout.
  - **🧪 Synced sample incentives render in full on the storefront.** A slug-synced SAMPLE variant now carries its FULL sampler definition + precomputed price math as a self-contained `samplerConfig` snapshot (written by `commitInventorySyncSlug` via the new `samplerPresentationSeed()`), preserved through `normalizePriceCategory` (`loadProducts`) AND `sanitizeProduct` (`/api/store`). New `lib/sampler-config.ts` helpers `resolveCategorySamplerConfig()` and `categorySamplerPresentation()` resolve the presentation from the SELECTED VARIANT OBJECT (blob first, product-level fallback), and `components/Storefront.tsx` now calls `categorySamplerPresentation(product, selectedCategory)` — so a synced sample shows its badge, credit math and note exactly like its standalone source page, with zero cross-product lookup. `categorySampleBadge` also falls back to `samplerConfig.label`.
  - **🧪 Verified:** `npx tsc --noEmit` clean, `npm test` **353/353** (3 new `tests/sampler-config.test.ts` cases), `eslint` 0 errors on all touched files (`app/admin/page.tsx` still emits only the known BABEL "file exceeds 500KB" note). No new Redis keys — `samplerLabel`/`samplerConfig` live on the variant object inside `store:products`.

- **2026-09-05 — Dynamic variant selection (index-based) + admin SKU re-hydration (`dynamic-variant-selection-sku-rehydration`):**
  - **🎯 Storefront variant-selection collision.** `components/Storefront.tsx` tracked the selected variant by its `size` STRING and resolved price/checkout-mode/limits via `cats.find(c => c.size === selectedSize)`. When two variants shared the SAME size label (e.g. one SKU offered as a raffle allocation AND an instant buy), every chip + resolver collapsed onto the FIRST match, so price, mode, limits and the countdown were all read from the wrong variant. The selection state is now `selectedVariantIndex` (a number); the canonical derivation is `activeVariantIndex` (clamped) → `selectedCategory` (the exact variant object) → `selectedSize` (string, still sent to checkout for backward compatibility), and the chips select by index with a unique composite key.
  - **🧬 New category-object resolvers (no size-string lookup).** `lib/checkout-mode.ts` gained `getCategoryCheckoutMode(product, category)` and `resolveCategoryLimits(product, category)` — they read the variant object directly, so duplicate-size variants each resolve their OWN mode and limits. `getSizeCheckoutMode` / `resolveSizeLimits` are now thin wrappers that find the category by size then delegate, preserving every existing caller (`server-config`, tests) byte-for-byte. Both helpers are re-exported from `lib/storefront-config`.
  - **🏷 Dynamic label disambiguation.** New `variantSizeLabel()` (Storefront) / `variantLabel()` (ProductLivePreview) append ` (price) - mode` to a variant's label ONLY when its size string is duplicated across the array, so two identical labels can never be confused. Zero hardcoded values — everything derives from the variant's own `size` / `price` and its resolved mode.
  - **👁 Admin live preview kept in sync.** `components/ProductLivePreview.tsx` (the admin editor's pixel-faithful storefront preview) had the identical `selectedSize === cat.size` collision; it now uses the same index-based selection + category resolvers + label disambiguation.
  - **🔑 Admin SKU re-hydration.** `lib/server-config.ts` `normalizePriceCategory()` returned a fresh object that silently DROPPED `sku` — and `loadProducts()` (used by `GET /api/admin/products`) maps every category through it, so the admin edit panel received variants with no `sku` and the SKU input (bound to `value={cat.sku || ''}`) re-opened blank. The field is now preserved verbatim (never dropped, never defaulted) so existing SKUs re-populate on edit.
  - **🧪 Verified:** `npx tsc --noEmit` clean, `npm test` **350/350** (4 new `tests/size-checkout-mode.test.ts` cases: duplicate-size mode + limit resolution by object, and product-level fallback), `eslint` 0 errors on all touched files. No new Redis keys.

- **2026-09-05 — Preserve slug-synced variants with a blank local size in `saveProduct()` (`synced-variant-blank-size-retention`):**
  - **🎯 Root cause of the 3 → 2 variant drop.** F12 showed `[DOM ROW SELECTOR COUNT]` = 3 and `[REACT STATE CAT COUNT]` = 3, but `[SAVING VARIANT COUNT]` fell to 2 — the third (synced) variant vanished from `[MULTI-VARIANT RECONCILED]` and `[CLIENT SAVE PAYLOAD]`. The culprit was the empty-size guard in `saveProduct()` (`app/admin/page.tsx`): `.filter((c) => String(c?.size || '').trim())` silently discarded ANY variant whose local `size` string was blank — including slug-synced variants, which legitimately inherit their size/name from the shared inventory pool slug and therefore often have no manually typed local size.
  - **🧬 Synced variants are now preserved regardless of local size.** The filter is replaced with a scoped `isSyncedCategory(c)` predicate (true when `syncWithExisting === true` or any of `inventorySyncSlug` / `_syncDraft` / `inventoryPoolId` is non-empty) and becomes `.filter((c) => String(c?.size || '').trim() || isSyncedCategory(c))` — a pool-linked variant can never be discarded for a missing local size string.
  - **🏷 Fallback size populated for synced rows.** Inside the `effectiveSlug` branch, the pool source is now resolved ONCE (`findInventorySyncSource`) and shared by the price, SKU **and** size fallbacks. When `out.size` is empty it is backfilled in priority order: the source category's own size → `humanizeSlug(effectiveSlug)` (new helper, `black-tee` → `Black Tee`) → the neutral `'Synced Pool'` label. A `syncWithExisting`-flagged row with no resolvable slug also gets `'Synced Pool'` in the `else` branch so it is preserved rather than dropped.
  - **🧪 Verified:** `npx tsc --noEmit` clean, `npm test` **346/346**, `eslint app/admin/page.tsx` exit 0 (only the BABEL "file exceeds 500KB" note). No new Redis keys (the link fields remain `inventorySyncSlug`/`inventoryPoolId`; the fallback `size` is just a category field).
- **2026-09-05 — Fix variant truncation in `saveProduct()`: remove size-keyed dedupe + exempt synced variants from `duplicate_size` (`fix-variant-truncation-1-1-retention`):**
  - **🎯 Root cause of the 2 → 1 array collapse.** `saveProduct()` (`app/admin/page.tsx`) logged `[DOM ROW SELECTOR COUNT]` = 2 and `[REACT STATE CAT COUNT]` = 2, then `[SAVING VARIANT COUNT]` dropped to 1 and `[CLIENT SAVE PAYLOAD]` emitted only 1 variant whenever a sync slug was involved. The culprit was a **size-keyed dedupe** between the reconciliation sweep and the payload map: `.filter((c) => { const key = size.toLowerCase(); if (seenSizes.has(key)) return false; ... })` silently dropped the second variant when two variants shared a size label — which is exactly what happens when two variants link to the SAME shared-inventory slug/pool (`inventorySyncSlug` / `inventoryPoolId`), e.g. the same SKU offered as a raffle allocation AND an instant buy drawing from one stock pool.
  - **🧬 1:1 variant retention enforced.** The `seenSizes` dedupe is removed. `priceCategories` is now `reconciledCategories.filter(empty-size).map(...)` — the ONLY remaining filter drops rows with no size label (an incomplete placeholder, not a sellable variant). Every variant in `productForm.priceCategories` is preserved 1:1 with its full metadata (size, price, stripeId, winnerTiers, position, SKU, units) alongside its individually resolved slug (or `null`).
  - **🛡 Downstream guard aligned.** `lib/product-sanity.ts` `checkProductSanity` previously emitted a `duplicate_size` **error** for any two same-size variants (no sync exemption), which would have re-blocked the save the moment the client dedupe was removed. The check now skips synced variants — `isSyncedVariant = Boolean(inventorySyncSlug.trim() || inventoryPoolId.trim())` (moved up in the loop and reused by both the `duplicate_size` and `empty_price` gates) — so shared-pool variants are never false-flagged, while independent variants still can't share a size label.
  - **🧪 Verified:** `npx tsc --noEmit` clean, `npm test` **346/346** (new `tests/product-sanity.test.ts` case: two variants sharing a sync slug may share a size — no `duplicate_size`), `eslint app/admin/page.tsx` / `lib/product-sanity.ts` / `tests/product-sanity.test.ts` exit 0. No new Redis keys.

- **2026-09-05 — Broadened variant DOM scraper + DOM/state diagnostics (`broaden-variant-dom-scraper-diagnostics`):**
  - **🧱 `scrapeVariantRowsFromDom()` no longer depends on `[data-variant-row]` alone.** New `queryVariantRowElements()` resolves rendered variant rows via a three-tier fallback: (1) the canonical `[data-variant-row]` attribute (index-keyed), (2) structural selectors `tr.variant-row, div.variant-row, .variant-card, .grid, .flex` filtered by a new `isLikelyVariantRow()` heuristic (a row is "variant-shaped" when it holds a size/price/Stripe input), and (3) grouping sellable inputs (`pf-size-*` / `pf-price-*` / Stripe-placeholder) by their closest `tr, div.grid, div.flex, .border` container. Non-indexed fallback rows derive their index from DOM order; the size/price lookups now fall back from the exact `pf-size-<idx>` / `pf-price-<idx>` id to any `input[id^="pf-size-"]` / `input[id^="pf-price-"]` within the row.
  - **📡 New diagnostics in `saveProduct()`.** Immediately before the DOM reconciliation, the client now logs `[DOM ROW SELECTOR COUNT]` (raw `[data-variant-row]` count) and `[REACT STATE CAT COUNT]` (`productForm.priceCategories.length`) so an operator can verify in F12 whether a rendered variant has drifted out of master state.
  - **🔍 Audited the "+ Add Variant" flow (unchanged — already correct).** `addPriceCategory` synchronously appends into `productForm.priceCategories` via a functional `setProductForm` and re-indexes `position`, and every rendered variant (initial + newly added) already carries `data-variant-row={idx}` on its outer container.
  - **🧪 Verified:** `npx tsc --noEmit` clean, `npm test` **345/345**, `eslint app/admin/page.tsx` 0 errors (only the BABEL "file exceeds 500KB" note). No new Redis keys (the link fields remain `inventorySyncSlug`/`inventoryPoolId`; stock lives in `inventoryPerSize`).
- **2026-09-05 — Hard DOM variant replacement (scrape stock + REPLACE full array) + `[SAVING VARIANT COUNT]` telemetry (`variant-dom-replace-stock-scrape`):**
  - **🧱 `scrapeVariantRowsFromDom()` now also scrapes STOCK (units).** The `ScrapedVariantRow` shape gained a `stock: number | null` field, resolved from each row's `input[placeholder*="unit"]` (the per-size Units input). Full sellable field set is now: size, price, stripeId, SKU, stock, and the sync slug.
  - **🔁 REPLACE instead of APPEND on DOM/state mismatch.** In `saveProduct()`, when the LIVE DOM variant-row count exceeds `productForm.priceCategories.length`, the reconciliation now REBUILDS the entire `workingCategories` list from `scrapeVariantRowsFromDom()` (not just the missing tail). Rows that still line up with React state use the state category as the BASE (preserving `commerceMode` / `winnerTiers` / `maxPerEmail` / `maxPerCart` / `sizeConfig` / `position`), with the DOM-scraped sellable fields overlaid as authoritative; trailing rows are reconstructed from scratch. This guarantees a rendered variant can never be dropped, even when the dropped update was mid-list rather than at the tail.
  - **📦 Scraped stock folds into `inventoryPerSize`.** A new `workingInventoryPerSize` map carries DOM-scraped per-size stock into the payload, and `totalInventory` is now computed from `{ ...productForm, priceCategories, inventoryPerSize: workingInventoryPerSize }`, so a DOM-reconstructed variant never loses its Units on save.
  - **📡 Telemetry.** Added `console.log('[SAVING VARIANT COUNT]', priceCategories.length)` alongside the existing `[MULTI-VARIANT PAYLOAD COUNT]` / `[MULTI-VARIANT RECONCILED]` / `[DOM VARIANT RECONCILIATION]` (now reports `replaced` instead of `appended`).
  - **🧪 Verified:** `npx tsc --noEmit` clean, `eslint app/admin/page.tsx` exit 0, `npm test` **345/345**. No new Redis keys (the link fields remain `inventorySyncSlug`/`inventoryPoolId`; stock lives in `inventoryPerSize`).
- **2026-09-05 — DOM variant reconciliation (never drop a rendered variant) + per-variant field scraping in `saveProduct` (`dom-variant-reconciliation-append-missing`):**
  - **🧱 New `scrapeVariantRowsFromDom()` helper** (module-level, in `app/admin/page.tsx`). It iterates every rendered `[data-variant-row]` container and scrapes the FULL sellable field set straight from the LIVE DOM — size name (`input[id="pf-size-<idx>"]`), price (`input[id="pf-price-<idx>"]`), Stripe price ID (`input[placeholder*="stripe"]`), SKU (`input[placeholder^="sku"]`) — plus the shared-inventory sync slug using the SAME three-source priority as `scrapeSyncSlugsFromDom()` (badge → sync input → "Synced with <slug>" text). All resolution is STRICTLY row-scoped so a value can never be mis-assigned to a different variant.
  - **🛟 DOM VARIANT RECONCILIATION in `saveProduct()`.** At the top of the save flow, the client now counts rendered variant rows. When the LIVE DOM holds MORE rows than `productForm.priceCategories.length` (a variant was rendered but its master-state update was dropped — the "variant dropping" bug), it scrapes the missing rows' fields from the DOM and APPENDS reconstructed variant objects (`{ size, price, stripeId, winnerTiers: '1', position, sku, sync-slug }`) into a `workingCategories` list BEFORE reconciliation and payload construction. A rendered variant can never be silently dropped at save time; a DOM badge/input slug is carried onto the reconstructed variant too.
  - **🔗 Master-state binding verified (unchanged, already correct).** `addPriceCategory` synchronously appends into `productForm.priceCategories` via a functional `setProductForm`, and every input change (size / price / Stripe ID / SKU / sync mode) routes through `updatePriceCategory` / `setVariantCommerceMode` / `typeInventorySyncSlug` / `applyInventorySyncSlug`, each of which writes `productForm.priceCategories[i]` in master React state.
  - **📡 Telemetry.** A new `[DOM VARIANT RECONCILIATION]` log reports `domVariantCount` / `stateVariantCount` / `appended` whenever a mismatch is repaired, alongside the existing `[MULTI-VARIANT PAYLOAD COUNT]` / `[MULTI-VARIANT RECONCILED]` / `[CLIENT SAVE PAYLOAD]` logs.
  - **🧪 Verified:** `npx tsc --noEmit` clean, `npm test` **345/345**, `eslint app/admin/page.tsx` exit 0. No new Redis keys (the link fields remain `inventorySyncSlug`/`inventoryPoolId`).
- **2026-09-05 — Multi-variant payload preservation + row-scoped DOM slug resolution in `saveProduct` (`multi-variant-payload-row-scoped-dom-slug`):**
  - **🧬 Every variant survives reconciliation.** `saveProduct` maps over EVERY element of `productForm.priceCategories` (no truncation/limit) and always spreads `{ ...c }`, mutating ONLY the four sync fields (`inventorySyncSlug` / `inventoryPoolId` / `_syncDraft` / `syncWithExisting`); `size`, `price`, `stripeId`, `winnerTiers`, `position`, `sku`, `stock` and every other field are preserved verbatim.
  - **🎯 Row-scoped DOM slug resolution.** Each variant editor row now carries a stable `data-variant-row={idx}` attribute. `scrapeSyncSlugsFromDom()` iterates `[data-variant-row]` containers and resolves each row's slug from THAT row only (badge `[data-sync-badge-slug]` → `input[id^="pf-sync-slug-"]` → "Synced with <slug>" text scoped within the row) — never a page-wide scan that could mis-assign a slug to a different variant.
  - **🥇 Strict priority: state → DOM.** Reconciliation now resolves the slug as `stateSlug || domSlug` where `stateSlug` = `inventorySyncSlug || _syncDraft || inventoryPoolId` (highest authority) and `domSlug` is only a fallback. A committed/typed slug can never be clobbered by a stale DOM badge; an independent variant with no slug stays `slug = null`.
  - **📡 Multi-variant telemetry.** `saveProduct` logs `[MULTI-VARIANT PAYLOAD COUNT]` (final `priceCategories.length`) and the full `[MULTI-VARIANT RECONCILED]` array to F12 before the POST, so every variant's survival and slug can be verified.
  - **🧪 Verified:** `npx tsc --noEmit` clean, `npm test` **345/345**, `eslint app/admin/page.tsx` 0 errors. No new Redis keys (the link fields remain `inventorySyncSlug`/`inventoryPoolId`).
- **2026-09-05 — Scoped sync reconciliation + payload field retention in `saveProduct` (`sync-reconciliation-scope-field-retention`):**
  - **🔒 Removed the brute-force global slug recovery.** The "product-level sync-state inheritance" (`rootSyncSlug` copied down to EVERY variant), the "unconditional page-wide DOM extraction" (reading every `<input>` value + scanning `document.body.innerText` for `Synced with …`/`Inherits price …` markers), and the resulting `bruteForceSlug` were all deleted. They forced a single recovered slug onto ALL variants — including independent, non-synced variants — which could silently clobber a variant's own configuration.
  - **🎯 Reconciliation is now STRICTLY scoped.** `reconciledCategories` in `saveProduct` only touches a variant when it is (a) explicitly `syncWithExisting`-flagged, (b) already carrying an active `inventorySyncSlug` / `_syncDraft` / `inventoryPoolId`, or (c) showing a DOM-scraped slug at its OWN row index (`scrapeSyncSlugsFromDom()`). Independent variants are returned verbatim — no slug is ever forced onto them.
  - **🧬 Field retention guaranteed.** Reconciliation always spreads `{ ...c }` and mutates ONLY the four sync fields (`inventorySyncSlug` / `inventoryPoolId` / `_syncDraft` / `syncWithExisting`); the payload mapping continues to build each outgoing object from `{ ...c, position: i }`, so price, stripeId, winnerTiers, stock, sku, size, and every other variant field is preserved verbatim alongside the resolved slug.
...
- **2026-09-05 — Brute-force global slug recovery in `saveProduct` (`brute-force-global-slug-recovery`):**
  - **🧬 Product-level sync-state inheritance.** `saveProduct` now checks for a ROOT-LEVEL sync slug on the product form (`productForm.inventorySyncSlug` / `inventoryPoolId` / `_syncDraft`) and, when present, copies it down to EVERY variant before the payload is built — a product-scoped pool link can never be dropped just because no single variant re-stated it.
  - **🛰 Unconditional page-wide DOM extraction.** Inside `saveProduct`, the client now reads EVERY `<input>` value on the page (`Array.from(document.querySelectorAll('input')).map(i => i.value)`) and logs it as `[PAGE INPUT VALUES]` to F12. It also scans `document.body.innerText` for `Synced with <slug>` and `Inherits price …` markers, then falls back to the first slug-like input value. If ANY slug-like string is found anywhere on screen, `inventorySyncSlug` / `_syncDraft` / `syncWithExisting` are set to the same recovered slug on ALL variants.
  - **🧭 Priority order in reconciliation:** per-index DOM scrape → per-category draft/pool → PRODUCT-level inheritance → page-wide brute-force slug.
...
- **2026-09-05 — Nuclear DOM + badge reconciliation for the inventory-sync save flow (`inventory-sync-nuclear-dom-badge-reconcile`):**
  - **🧱 `scrapeSyncSlugsFromDom()`** — a new module-level helper that scrapes every visible shared-inventory sync slug straight out of the LIVE DOM, keyed by the ORIGINAL `priceCategories` row index, from THREE sources in priority order: (1) badge cards via new `data-sync-badge-idx`/`data-sync-badge-slug` attributes (committed slugs), (2) the dedicated `#pf-sync-slug-<idx>` inputs (typed-but-uncommitted drafts), and (3) a last-resort text scan of any element containing `Synced with <slug>` (resolving the row index from the nearest `[data-sync-badge-idx]` ancestor).
  - **🔗 Badge cards now carry `data-sync-badge-idx` + `data-sync-badge-slug`.** The "🔗 Synced with [slug]" card exposes its canonical slug + row index as data attributes so the save-time sweep can read the operator-visible slug directly instead of relying on React state that may have drifted.
  - **💾 `saveProduct()` reconciliation is now UNCONDITIONAL.** The pre-save sweep calls `scrapeSyncSlugsFromDom()` once, then for every variant overwrites ALL four sync fields (`inventorySyncSlug`, `inventoryPoolId`, `_syncDraft`, `syncWithExisting`) to the SAME slug whenever a slug exists in the DOM badge, the input, the draft, OR the pool id. A slug the operator typed OR merely saw in a badge can never be dropped on save. The payload map's `rawSlug` chain is the absolute last resort and is guaranteed non-null once reconciliation has run.
  - **🧪 Verified:** `npx tsc --noEmit` clean, `eslint app/admin/page.tsx` 0 errors, `npm test` **345/345**. No new Redis keys (the link fields remain `inventorySyncSlug`/`inventoryPoolId`).
- **2026-09-05 — Sync-slug state binding + DOM fallback hardening (`sync-state-binding-dom-fallback`):**
  - **⌨️ Every keystroke mutates master state.** `typeInventorySyncSlug` now writes `_syncDraft` AND `syncWithExisting: true` on EVERY keystroke (previously only `_syncDraft`), so a variant being edited can never drift back to an "unlinked" shape mid-edit. `inventorySyncSlug` stays committed-only (setting it on keystroke would flip `synced` and unmount the input after the first character).
  - **🔗 Stable DOM id on the sync slug input.** The "Inventory Sync Slug" `<input>` now carries `id={`pf-sync-slug-${idx}`}` so the save-time fallback can query it directly.
  - **🧱 DOM + draft fallback in `saveProduct()`.** Right before mapping the payload categories, a reconciliation sweep (`reconciledCategories`) reads the LIVE DOM input value per variant and — when `inventorySyncSlug` is empty but a DOM value, `_syncDraft`, or `inventoryPoolId` exists — promotes that string into `inventorySyncSlug` + `_syncDraft` and sets `syncWithExisting: true`. A typed-but-never-committed slug can never be silently dropped on save.
  - **🧪 Verified:** `npx tsc --noEmit` clean, `npm test` **345/345**. No new Redis keys (the link fields remain `inventorySyncSlug`/`inventoryPoolId`).
- **2026-09-05 — Fix disconnected "🔗 Synced with [slug]" badge vs payload builder (`badge-pool-id-fallback`):**
  - **🎯 Root cause.** The badge card + `synced`/`syncEnabled`/`isSyncEnabled` gates were driven ONLY by `cat.inventorySyncSlug`, while `saveProduct`'s payload builder resolved the slug from `c.inventorySyncSlug || c._syncDraft` — neither read the canonical `inventoryPoolId`. A variant whose pool identity lived only in `inventoryPoolId` (e.g. hydrated from a record that bound the pool id without the operator-facing slug) could still render a badge, yet the payload builder emitted `inventorySyncSlug: null` / `inventoryPoolId: null`, silently UN-LINKING the variant on save.
  - **🧱 One canonical resolver.** New `categorySyncSlug(cat)` helper resolves `inventorySyncSlug || inventoryPoolId` (slugified) and is now the SINGLE source of truth for the badge. The badge's `syncSlug`, `synced`, `syncEnabled`, and `isSyncEnabled` all fall back to `inventoryPoolId`, and `saveProduct`'s `rawSlug` now resolves `inventorySyncSlug || _syncDraft || inventoryPoolId` — so a rendered badge can never be un-linked by the payload builder.
  - **🧪 Verified:** `npx tsc --noEmit` clean, `eslint app/admin/page.tsx` 0 errors, `npm test` **345/345**. No new Redis keys (the link fields remain `inventorySyncSlug`/`inventoryPoolId`).
...
- **2026-09-04 — Products panel refactor for 10k+ SKUs: enum status, bulletproof prices, shared inventory pools, presigned CDN uploads, paginated/batch PM (`products-panel-refactor`):**
  ...
  - **🔗 Shared Inventory Pools.** New `lib/inventory-pool.ts` binds each size's `inventorySyncSlug` to a canonical `inventoryPoolId` (write-time) and resolves `shared:<poolId>` fields in `ops:live_state` — reusing the existing `lib/checkout-mode.ts` helpers so checkout/draw/webhook decrements are untouched. `/api/admin/products` binds pools on save.
  - **☁️ In-panel CDN uploads (deprecate base64).** New `app/api/admin/media/presign/route.ts` issues AWS-SigV4 presigned PUT URLs (Node `crypto` only — works for S3 and R2) under `products/<slug>/<uuid>.<ext>`. The admin gallery gained a native drag-and-drop dropzone and `handleImageFiles` uploads via presigned URL first (persisting `publicUrl`), falling back to the legacy base64 `/api/admin/upload` when storage is unconfigured (env: `MEDIA_BUCKET`, `MEDIA_S3_ACCESS_KEY_ID`, `MEDIA_S3_SECRET_ACCESS_KEY`, `MEDIA_S3_REGION`, `MEDIA_S3_ENDPOINT`, `MEDIA_S3_PUBLIC_BASE_URL`).
  - **⚡ High-volume PM.** New `lib/product-query.ts` (pure pagination + fuzzy search over title/SKU/slug + faceted status/category/checkoutMode/hasInventoryPool filters). `/api/admin/products` GET is paginated when `?page=` is present (backward-compatible otherwise). New `app/api/admin/products/batch/route.ts` adds multi-select batch `setStatus` / `setCategory` / `setInventorySync` and CSV export (`?export=csv`).
  - **🧪 Tests:** `tests/product-status.test.ts`, `tests/price-validation.test.ts`, `tests/inventory-pool.test.ts`, `tests/product-query.test.ts` (28 new cases). `npm test` **328/328**, `tsc --noEmit` clean, `eslint` clean. No new Redis keys (status lives inside `store:products` records; pools reuse `shared:<slug>`).

- **2026-09-04 — Remove redundant AI settings assistant, make FCFS×winners impossible, shared inventory slugs (`remove-ai-assistant-fcfs-sanity-shared-inventory`):**
  - **🧹 Removed the "AI Settings Assistant" card** from Settings (and its `/api/admin/ai-settings` route). The top-of-portal **Admin Portal Helper** (`/api/admin/ai-helper`) is the single AI surface now — the settings-only assistant was redundant.
  - **🚫 FCFS sizes can never carry winners anymore.** `app/api/admin/products` strips `winnerTiers` from any FCFS size on save, and the admin editor clears it the moment a size (or the product) flips to FCFS — including on load/duplicate/reset — so the "winners on FCFS" warning can never be produced instead of just being flagged. The pure engine keeps the `winners_on_fcfs` warning as a last-resort safety net.
  - **🔗 Shared inventory via slug.** Each size in Pricing, Sizes & Inventory gained an optional **"Sync slug"** field. Sizes (in any product) that share the same slug draw from ONE shared stock pool, stored as a `shared:<slug>` field inside the existing `ops:live_state` hash — so checkout/draw/webhook decrements just work across products with no per-call-site changes. `lib/checkout-mode.ts` gained pure helpers (`normalizeInventorySyncSlug`, `resolveInventorySyncSlug`, `sharedInventoryField`); `aggregateLiveInventoryByProduct`/`findLiveInventoryForProduct` skip + fold pools correctly, `redis-maintenance` never orphans shared pools, and the self-test resolves shared fields.
  - **🧽 Product panel cleanup:** tightened the "Pricing, Sizes & Inventory" description and fixed the stale "winner count set below" copy.
  - **🧪 Verified:** `npx tsc --noEmit` clean, `eslint` clean on all touched files, `npm test` **300/300** (added shared-inventory helper coverage).
- **2026-08-30 - Phases 1-8 completion pass (`phase-1-8-auth-2fa-product-settings-email-ai`):**
  - **Phase 1 - env-check cleanup:** `lib/env-discovery.ts` now marks the legacy env-var provider checks (Basic Auth password/verify email, Stripe secret + webhook, cron secret) as `required: false`. These are superseded by the Setup Wizard (Supabase `global_platform_settings`), so the SetUp tab's "Missing required" summary no longer reports false failures for `STRIPE_SECRET_KEY` / `STRIPE_WEBHOOK_SECRET` / `ADMIN_BASIC_AUTH_PASSWORD` / `CRON_SECRET` etc. `tests/env-discovery.test.ts` updated to match.
  - **Phase 2 - customer 2FA:** `app/api/auth/signup/route.ts` now (1) re-sends the verification code instead of returning a 400 "User already exists" when retrying signup with an unverified email, (2) respects a new `store:config.requireSignup2FA` toggle (default ON), and (3) surfaces a clear 502 when the 6-digit code email fails to send instead of silently reporting success. `app/api/auth/resend-verification/route.ts` returns 429 (throttle) / 502 (send failure) instead of a bare 500. `lib/customer-verify.ts` improved the production send-failure message to flag the unverified Resend FROM-domain cause. New admin toggle "Require email verification (2FA) at customer signup" under Settings -> Rewards & Points (`app/admin/page.tsx` + `app/api/admin/settings/route.ts`).
  - **Phase 3 - product panel:** the per-size checkout-mode dropdown in Pricing, Sizes & Inventory now shows "Follow product (RAFFLE/FCFS)" instead of the ambiguous "Auto (product)", so the effective mode is always visible. Added "Save as default" in the product editor: persists the current checkout mode, size template and delivery-incentive defaults to `store:config.productDefaults`, and `resetProductForm()` pre-fills new products from it.
  - **Phase 4 - checkout 500:** verified the prior root-cause fix (settings-read guard in `services/config/*`) is present; no further change needed.
  - **Phase 5 - settings auto-save bug:** split `settingsLoading` (fetch) from a new `settingsSaving` (save) state in `app/admin/page.tsx`, so the "Save All Settings" buttons no longer flash "Saving..." the moment the Settings tab opens (which only loads).
  - **Phase 7 - email sender image + main-page AI animation:** `lib/env.ts` gained `getBrandLogo()` (`EMAIL_LOGO_URL` -> `BRAND_LOGO_URL` -> `NEXT_PUBLIC_LOGO_URL`, URL-only). `lib/email.ts` prepends the store logo to every transactional email masthead (via the `getResend()` shim) and passes it through to 2FA code emails; `services/email/types.ts` + all three drivers accept a `logoUrl` in `CodeEmailOptions`/`buildCodeEmailHtml`. `app/page.tsx` now renders a subtle AI "slow drift" animation (the `/api/ai/animation` fallback preset) on the featured product's cover image at the top of the home-page hero.
  - **Verified:** `npx tsc --noEmit` clean, `eslint` clean, `npm test` 288/288 (added `tests/env.test.ts` `getBrandLogo` case), `npm run build` succeeds. No new Redis key namespaces (reused `store:config.requireSignup2FA` + `store:config.productDefaults`).

- **2026-08-30 — Admin setup lockdown + checkout 500 fix + product-panel/Home-page fixes (`admin-setup-lockdown-checkout-product-panel`):**
  - **🔒 Setup menu now fully locked after admin creation (Phase 1).** `middleware.ts` no longer exempts `/admin/setup` (page) or `/api/admin/setup` (GET/POST) from Gate 1 (Basic Auth / login session / device cookie) or Gate 2 (2FA) once the store is ready — the setup wizard is only reachable pre-configuration (the `!ready` readiness gate still returns `next()` for it). Removed the now-unused `isSetupRead` / `isSetupApi` flags; `isSetupReconfigure` is still consulted by the "ready" redirect so a signed-in master admin reaches the reconfigure entry point through `/admin/login`. `app/admin/login/page.tsx` no longer links to `/admin/setup?reconfigure=1` during sign-in.
  - **✨ Setup flicker eliminated.** `app/admin/setup/page.tsx` now renders a neutral loading placeholder while the `GET /api/admin/setup` readiness probe is in flight, so the customizable wizard no longer flashes for a fraction of a second before flipping to the "Store already set up" screen.
  - **🛒 Checkout 500 "Checkout could not be started" root-caused (Phase 4).** `fetchPlatformSettingsRow()` (`services/config/supabase-client.ts`) threw straight through `PaymentFactory.getDriver()` whenever Supabase was configured but its `global_platform_settings` read failed (missing table, bad service key, network blip) — so a legacy env Stripe key was never reached and the raffle/FCFS checkout route returned a generic 500. It now catches and returns `null`, and `getPlatformSettings()` (`services/config/platform-settings.ts`) also guards a malformed-row/parse throw, so payment/email factories fall back to env keys instead of 500ing.
  - **🖼 Product-panel fixes (Phase 2).** `app/admin/page.tsx` `editProduct()` now resets transient editor state (crop editor index, image URL input, note editor, status message) when switching which product is edited — fixing the stale-state glitch when switching between products of different shapes. The media gallery gained a **reorder** control (◀ ▶ per thumbnail) via a new `moveImage()` helper that keeps the per-image crop list index-aligned, and the first image is labeled "cover".
  - **🏠 Home-page fallback override (Phase 2).** New `layout.homepageFallback` store-config setting (admin → Settings → Home Layout): `none` / `upcoming` / `archived` / `upcoming_then_archived` (default `upcoming`). `app/page.tsx` + `components/Storefront.tsx` now surface upcoming (then archived) releases when there are no active ones, so the home page never renders blank while drops sit hidden. Upcoming items already list fine without a release date (the lifecycle flag is date-independent).
  - **🧪 Verified:** `npx tsc --noEmit` clean, `eslint` 0/0 on the touched non-page files, `npm test` **287/287**. No new Redis keys added (the fallback lives in the existing `store:config.layout`).

- **2026-08-30 — Real AI hero animation + locked entry email + bounded AI settings assistant (`real-ai-hero-email-lock-ai-settings-assistant`):**
  - **🎬 Real AI main-page animation (no CSS-drift workaround).** New public `GET /api/ai/hero-animation` resolves the featured product's cover image, then calls the actual `services/ai` engine (`AiFactory.getDriver()` → `buildAnimationPrompt` → `parseAnimationResult`) to generate real CSS keyframes / SVG, cached for 24h under a new `cache:ai-hero:<productId>` key (fingerprinted by the cover image so swapping the photo regenerates). Only falls back to the built-in drift preset when no AI provider is configured or the call fails. `app/page.tsx` fetches this and injects the AI keyframes/SVG into the hero instead of hardcoding `fallbackAnimation('drift')`.
  - **🔒 Entry email locked for signed-in customers.** `components/Storefront.tsx` now fetches `/api/auth/me` on mount and, when a session exists, locks (disables + greys out) the Raffle/FCFS entry email field to the account email so an alternate address can't be submitted.
  - **🤖 Bounded AI settings assistant.** New admin-only `POST/GET /api/admin/ai-settings` maps a natural-language instruction onto a strict allowlist of `store:config` boolean toggles (`requireSignup2FA`, `checkout.requireAddressAutofill`, `behavior.scrollToTopOnLoad`, `socialProof.showSection`, `socialProof.showCaption`) and re-validates every change so a hallucinated key can never mutate settings outside the operator's permission surface. Added a small "AI Settings Assistant" card to the Settings tab.
  - **🧪 Verified:** `npx tsc --noEmit` clean, `eslint` clean on all touched files, `npm test` **293/293** (added `tests/ai-hero-animation.test.ts`), `npm run build` succeeds (both new routes compiled).

- **2026-08-29 — Provider-key saves now self-heal the schema inline + partial saves no longer demand an AI key (`provider-keys-inline-access-token-self-heal`):**
  - **🐛 Root cause of the live 400s.** On a production store the Supabase schema was never applied (or was wiped), so `POST /api/admin/provider-keys` hit two failures: (1) with all keys filled, `savePlatformSettings()` threw a PostgREST schema gap → the route returned “Your Supabase database is missing part of its schema… run `supabase db push`”; (2) with only some keys, `getPlatformSettings()` returned `null` (schema gap), so `normalizePlatformSettingsPatch(body, null)` could not merge the stored write-only keys and demanded “Enter an AI provider API key (Workers AI needs none)” for the selected-but-blank AI provider. The old self-heal only ran when `SUPABASE_ACCESS_TOKEN` was set as an **environment variable** — which the operator had never set — so it never fired.
  - **🔑 Inline Supabase access token now accepted.** The admin **“API Keys & Integrations”** panel and the Setup Wizard Step 1 both gain an optional “Supabase access token (`sbp_…`)” field. It travels with the save (`supabase_access_token`) and is kept in a runtime override (new `setSupabaseRuntimeAccessToken()` / `readSupabaseAccessToken()` in `services/config/supabase-client.ts`), so the store can run the Management-API migrations itself without the operator touching a terminal or redeploying. `lib/supabase-migrate.ts` (`autoApplySchema` / `supabaseAutoMigrateAvailable` / `isMalformedSupabaseAccessToken`) now takes an optional token override; the Setup Wizard `probeAutoMigrate` + main POST use it too.
  - **🗄 Self-heal now runs BEFORE normalization.** `/api/admin/provider-keys` reads the existing row first; if that read throws a schema error it auto-applies the migrations (when a token is available) and re-reads, so partial key updates merge the stored values instead of erroring. The write path still self-heals for the column-level gap (`stripe_price_id` / `ai_api_key_secondary`) and now surfaces the real migration error when a token is supplied but rejected.
  - **🧪 Verified:** `npm test` **287/287** (2 new `tests/supabase-env.test.ts` access-token resolution cases), `tsc --noEmit` clean, `eslint` 0/0 on touched files, `npm run build` compiles all routes (incl. `/api/admin/provider-keys` + `/api/admin/setup`).

- **2026-08-29 — Partial provider-key saves + no more 500/empty-body in "API Keys & Integrations" (`provider-keys-partial-save-and-error`):**
  - **🐛 Root cause of "Failed to load resource … 500" + "Could not save provider keys: … Unexpected end of JSON input".** `POST /api/admin/provider-keys` called `savePlatformSettings()` / `getPlatformSettings()` with no try/catch, so any server error (a missing `SUPABASE_SERVICE_ROLE_KEY`, or a missing `stripe_price_id` column on an older DB whose `00005` migration was never applied) threw an unhandled exception → Next.js returned a 500 with an empty body → the client's `res.json()` threw "Unexpected end of JSON input". The route now wraps the save in a try/catch and returns a JSON `{ error }` (with a `schemaError` fix-plan when PostgREST reports a schema gap), and the admin client parses the body defensively so it always shows the real message (or `HTTP nnn`).
  - **🗄 Self-heal on save.** `saveWithSchemaHeal()` retries via `autoApplySchema()` when `isSchemaError()` matches and `SUPABASE_ACCESS_TOKEN` is present — mirroring the Setup Wizard — so a stale DB missing `stripe_price_id` / `ai_api_key_secondary` is patched automatically before the write.
  - **🔑 Partial key updates now work.** API keys are write-only (never echoed back), so a re-save used to fail with "Enter a … API key" unless every configured provider's key was re-entered — and would otherwise wipe the blank keys. New `normalizePlatformSettingsPatch(raw, existing)` (in `services/config/platform-settings.ts`) merges blank key fields with the stored values: blank + unchanged provider → keep; blank + a newly selected provider → require a key; provider blank/`none` → clear the whole category. Secret optional fields (e.g. Stripe webhook secret) are preserved when blank; non-secret `stripe_price_id` is cleared when blank. The `/api/admin/provider-keys` route now uses it, and the UI copy explains "leave a key blank to keep the saved value".
  - **🧪 Verified:** new `tests/provider-keys-patch.test.ts` (6 cases), `npm test` **285/285**, `tsc --noEmit` clean, `eslint` 0/0 on touched files, `npm run build` compiles all routes (incl. `/api/admin/provider-keys`).
- **2026-08-29 — Default Stripe price ID in "API Keys & Integrations" + fixed admin refresh bouncing to `/admin/setup` (`stripe-default-price-id-and-setup-fix`):**
  - **🐛 Root cause of "clicked 🔄 Refresh → sent to `/admin/setup`".** `savePlatformSettings()` (`services/config/platform-settings.ts`) passed `is_configured: false` in its PostgREST `resolution=merge-duplicates` upsert. Saving provider keys from `/admin` therefore flipped an already-configured store back to `is_configured = false`; the next request hit the middleware readiness gate, saw `platformConfigured === false` (and no Basic-Auth password for Supabase-only installs), returned `423 SETUP_REQUIRED`, and the admin client bounced to `/admin/setup`. The upsert now **omits** `is_configured` entirely so the existing flag is preserved (only `markPlatformConfigured()` may flip it). This also fixes the keys appearing to be lost after that refresh (the save was being followed by a forced redirect off the portal).
  - **➕ Default Stripe price ID field.** Added `stripe_price_id` to `global_platform_settings` (new idempotent `supabase/migrations/00005_stripe_price_id.sql`, also added to `00001_init.sql` for fresh installs, and to the parallel `multi-tenant-platform` schema). Wired end-to-end: `types.ts` (`GlobalPlatformSettings`, `PlatformSettingsInput`, `PlatformSettingsPublicSummary`, `parseSettingsRow`, `toPublicSummary`), `normalizePlatformSettingsInput`, `savePlatformSettings`, the Supabase `probePlatformSettingsSchema` select list, plus the admin **"API Keys & Integrations"** UI (a non-secret `price_...` input shown under Payments when Stripe is selected) and the Setup Wizard's optional payments card. The new `resolveDefaultStripePriceId()` / `resolveStripePriceIdWithSettings()` (async, DB → `STRIPE_PRODUCT_ID` → `''`) now drives checkout in `app/api/checkout/direct` + `app/api/admin/trigger-drop`, and `/api/admin/products` GET pre-fills the product form with the admin-saved default.
  - **🧭 Schema-guide + auto-migrate now cover 00005.** `lib/setup-schema-guide.ts` embeds `MIGRATION_00005`, adds a targeted `stripe_price_id` fix plan (run only 00005), and the full plan now lists five migrations (`00001`→`00005`); `lib/supabase-migrate.ts` auto-applies all five.
  - **🧪 Verified:** `npm test` **279/279** (new `tests/drivers.test.ts` price-ID parse/summary cases + `tests/setup-normalize.test.ts` pass-through case + `tests/setup-schema-guide.test.ts` 00005 byte-for-byte + targeted-plan cases), `tsc --noEmit` clean, `eslint` 0/0 on touched files, `npm run build` compiles all routes.
- **2026-08-28 — Idempotent super-admin creation so a leftover `auth.users` record no longer 422s setup (`setup-email-exists-idempotent`):**
  - **🐛 Root cause of `Supabase auth POST /admin/users failed (422): email_exists`.** Supabase keeps Auth accounts in the separate `auth.users` schema — NOT the `public` schema. "Clearing the database" (truncating `public.profiles` / `public.global_platform_settings`, or the app's Redis/`public`-only wipe) leaves `auth.users` intact, so `is_configured` resets to false while the master account still exists. The wizard then re-entered the first-run branch and re-called `createSuperAdmin()` → GoTrue `POST /auth/v1/admin/users` → `email_exists`, permanently blocking "Finish setup".
  - **🔧 Fix — `createSuperAdmin()` is now idempotent.** On `email_exists` it locates the existing Auth user by email (paginating GoTrue `GET /admin/users?page=&per_page=`, since that endpoint supports pagination but NOT email filtering — new `findAuthUserByEmail()`), refreshes that user's `password` + `user_metadata` (so a newly typed password still works after a DB reset), and re-links the `profiles.is_super_admin` flag — instead of failing. `supabaseAuthFetch()` gained a `PUT` method for `PUT /admin/users/:id`.
  - **🔒 Setup page self-closes once configured.** `middleware.ts` already redirects `/admin/setup` → `/admin` (which then requires the login gate) when `is_platform_configured()` is true. With the idempotent fix the wizard now completes and flips that flag, so random visitors are bounced to the login page; the provider/settings surface lives in `/admin` (+ `/admin/setup?reconfigure=1` for the signed-in owner). No new page was needed.
  - **🧪 Verified:** `npx tsc --noEmit` clean, `eslint` 0/0 on the touched files, `npm test` **272/272** (new `tests/super-admin.test.ts` cases: normal create + `email_exists` re-link). No Redis keys or DB columns added.
- **2026-08-26 — Missing service-role key no longer blocks admin login (auth-config-exempt):**
  - **🔓 Login routes gate on AUTH-capable config, not the service-role key.** The Supabase service-role key is only required for write operations (persisting settings, creating the admin, syncing profiles) — it is NOT needed to run the GoTrue password grant used to verify an operator's email+password. Previously `/api/admin/super-login` returned `503 "Supabase is not configured"` whenever `supabaseServiceConfigured()` was false, locking an operator out even when the URL + anon key (sufficient for login) were present.
  - **🔧 Fixes.**
    - `services/config/supabase-client.ts`: new `supabaseAuthConfigured()` (doc-only alias of `supabaseConfigured()`) + `supabaseAuthMissingReason(envOnly?)` which precisely reports WHY the Supabase auth path is unusable (no credentials at all / missing URL / missing anon key) and returns `null` when url + anon key are both present. It never reads the service-role key.
    - `app/api/admin/super-login/route.ts`: removed the service-role `503` hard block (replaced with an explanatory comment). Login now proceeds whenever auth-capable config exists, and the success response still carries `noticed: !supabaseServiceConfigured()` so the operator is *warned* (never blocked) that writes will fail until the service-role key is set.
    - `app/api/admin/login/route.ts`: same philosophy on the in-site login form — it only returns `503 admin_login_not_configured` when BOTH the Supabase auth path AND the legacy `ADMIN_BASIC_AUTH_PASSWORD`/`ADMIN_VERIFY_EMAIL` fallback are unusable. When any login method is available it falls through and returns a genuine `401` for wrong credentials.
  - **🧪 Verified:** `npx tsc --noEmit` clean, `eslint` 0/0 on all three files, `npm test` **268/268**. No Redis keys added or changed. Design decision: a missing service-role key is a warning (`noticed: true`), never a login blocker.

- **2026-08-26 — In-site admin login unblocked on unconfigured stores (`login-readiness-exempt`):**
  - **🐛 The readiness gate intercepted `/admin/login` before Gate 1/2 could apply.** When the install was `!ready` (no data store and/or no admin method yet), `middleware.ts`'s readiness gate (`if (!ready)`) only exempted `isSetupPath || isSuperLoginPath`. A request to `/admin/login` or `POST /api/admin/login` therefore returned `423 SETUP_REQUIRED` (or redirected to `/admin/setup`) BEFORE the Gate 1 (Basic Auth / login-session cookie / device cookie) and Gate 2 (2FA) exemptions — that are all gated on `isLoginPath` at lines ~347/363/382 — ever got a chance to run. The in-site login form was unreachable on a store that isn't yet configured, even though it is the replacement for the native Basic-Auth dialog and the only way in when `ADMIN_BASIC_AUTH_PASSWORD` is unset.
  - **🔧 Fix:** Added `isLoginPath` to the readiness-gate exemption list (`if (isSetupPath || isSuperLoginPath || isLoginPath)`) so the login page + API stay open pre-config, matching how setup and super-login already behave, and expanded the comment block above the gate. The route keeps its own self-auth (`/api/admin/login` verifies the Supabase super-admin OR the legacy Basic-Auth password via `verifySecureAdminAccount`-style logic, then mints the short-lived `goyunir_admin_auth` session cookie), so no write path is exposed — credential validation never relied on the middleware gate.
  - **🧪 Verified:** `npx tsc --noEmit` clean, `eslint middleware.ts` 0/0 on the touched file. No Redis keys added or changed. The affected file is `e-commerce-store/middleware.ts`.

- **2026-08-26 — Database picker in setup + in-site admin login (email, not username) + re-save deadlock fix (`db-picker-in-site-login`):**
  - **🗄 Setup step 1 now asks WHICH database(s) to use (two dropdowns).** `/admin/setup` step **1 · Choose your database** has a **Primary database** dropdown (Supabase / Upstash Redis / Cloudflare KV-D1) and a **Second database (safety mirror, optional)** dropdown. After a selection, an adaptive panel (`DatabaseInstructions`) shows the exact env-var names (`STORAGE_PROVIDER`, `STORAGE_REPLICAS`, `UPSTASH_REDIS_REST_URL`/`TOKEN`, Supabase trio), click-to-copy `npx wrangler secret put …` commands, "where to find them" instructions and dashboard links — the same copy-paste pattern the old Supabase-only panel used. Supabase stays its own **2 · Connect Supabase (admin & settings)** card (the master admin account + `global_platform_settings` always live there), and the admin-account card is now step **3**.
  - **♻️ Persisted both choices.** `OperationalSettings` gained `storage_replicas`, and both `storage_provider` / `storage_replicas` are persisted through `normalizeOperationalSettingsInput` → the `operational_settings` JSONB blob (mirrors the env-driven `STORAGE_PROVIDER`/`STORAGE_REPLICAS` runtime selectors). `tests/operational-settings.test.ts` updated.
  - **🔓 "Setup saved, then re-entering my admin account gives 422 + 401" FIXED.** The production `cloudflare_supabase_required` gate now only applies on FIRST setup (`!alreadyConfigured`), so re-saving an already-configured store no longer 422s. The re-configuration guard also now accepts the **master admin email + password** typed into step 3 (`verifySuperAdminSignIn`) as authorization, alongside Basic Auth / a super-admin session / the Supabase service-role key — closing the deadlock where the operator re-entered their admin account and got locked out.
  - **🔐 In-site admin login replaces the native Basic-Auth dialog.** New `/admin/login` page (email + password, "why two steps" explainer) + `POST /api/admin/login` (verifies the Supabase super-admin OR the `ADMIN_BASIC_AUTH_PASSWORD`, rate-limited, then mints a short-lived login session). `middleware.ts` now redirects unauthenticated `/admin`/`/api/admin` requests to `/admin/login` (`AUTH_REQUIRED`) instead of the browser's Basic-Auth popup, and Gate 1 accepts a login-session cookie OR a valid device cookie OR Basic Auth. New Redis keys `admin:auth:<token>` (TTL string) + `goyunir_admin_auth` cookie; new helpers in `lib/admin-verify.ts` (`issueAdminAuthSession` / `consumeAdminAuthSession` / `adminAuthTokenFromRequest` / `adminLoginAuthorized` / `resolveAdminLoginEmail`).
  - **📧 2FA is emphasized + wired to the login email.** The verify-* routes now authorize via the login session AND send the code to the **verified login email** (falling back to `ADMIN_VERIFY_EMAIL`/`SUPPORT_EMAIL`). The `/admin` 2FA gate copy now stresses why two-step matters and points the operator to set up a transactional email provider (Resend/Postmark/SendGrid) when codes aren't arriving.
  - **✉️ "username" → "email"** everywhere: the admin login form labels it **Email**, the middleware realm/comments already use email, and the README env-table + troubleshooting copy no longer mention a "username".
  - **🧪 Verified:** `npm test` **268/268**, `npx tsc --noEmit` clean, `eslint` 0/0, `npm run build` compiles (`/admin/login`, `/api/admin/login`, `/admin/setup` + middleware included).

- **2026-08-26 — Self-diagnosing Supabase connection + stupid-proof Cloudflare setup + redundancy surfaced (`supabase-live-probe`):**
  - **🔍 "Still 422 even after setting the 3 keys" is now impossible to be stuck on.** The 422 `cloudflare_supabase_required` gate is unchanged (the 3 Supabase values must be REAL env vars in production), but the wizard now has a **live "Test connection" button** that actually reaches Supabase and reports the truth: a new `probe: 'supabase'` POST mode in `/api/admin/setup` reads the 3 credentials (form → env), lists EXACTLY which of `SUPABASE_URL` / `SUPABASE_ANON_KEY` / `SUPABASE_SERVICE_ROLE_KEY` is still missing, and — once all 3 are present — hits `${url}/rest/v1/` with the service-role key to confirm the project is reachable AND the key is accepted (401/403 → "pasted the anon key instead of the service-role key"). Values are never echoed back. This surfaces the real cause (e.g. a `NEXT_PUBLIC_SUPABASE_*` build-time name set in the dashboard, which never appears in `process.env` at runtime on Cloudflare) instead of a bare 422.
  - **🧭 The "Connect Supabase first" banner is now a full walkthrough with BOTH options (no terminal needed).** It shows a direct **link to `https://dash.cloudflare.com/`** (new `CLOUDFLARE_DASHBOARD_URL` in `lib/env-discovery.ts`), the exact click-path (Workers & Pages → [project] → Settings → Variables and Secrets → Production → Secrets), the per-missing-key env-var name with a **"copy name"** button, the copyable `npx wrangler secret put <NAME>` command, and a "Check again" reload. `dataStoreSummary()` now also returns a `where` string per missing credential (from `cloudflareLocation()`).
  - **✍️ Pre-filled variable names + copy-paste everywhere.** Each of the 3 database fields in `/admin/setup` now shows the exact Cloudflare variable name as a copyable chip (`SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`) so a buyer just copies the name and pastes the Supabase value — plus a collapsible "Where do I find these values?" (supabase.com/dashboard → Project Settings → API → the 3 values). New `EnvNameChip` component.
  - **🛡 Redundancy surfaced in the wizard.** A new collapsed **"Optional: add a safety mirror (redundancy)"** card documents `STORAGE_REPLICAS` + `UPSTASH_REDIS_REST_URL`/`UPSTASH_REDIS_REST_TOKEN` (the existing write-through mirror engine in `lib/storage/replicated.ts`) so a future buyer can run off 2+ independent databases for failover — set as env vars in Cloudflare, not persisted through the wizard (correct: replicas are process-level env, not Supabase config).
  - **🧪 Verified:** `npm test` **268/268** (new `CLOUDFLARE_DASHBOARD_URL` case + a `where`-present assertion on `dataStoreSummary`), `npx tsc --noEmit` clean, `eslint` 0/0 on all touched files, `next build` compiles `/admin/setup` + `/api/admin/setup` cleanly. No Redis keys added or changed.

- **2026-08-26 — Setup wizard simplified to "less is more" (`setup-minimal-redesign`):**
  - **🎯 `/admin/setup` rebuilt as ONE calm page** (was a 1531-line, 5-step wizard with provider matrices, per-field copy commands, a Cloudflare "where your keys go" panel, schema-fix cards and env-var hints everywhere). The new page is **two required cards** — **1 · Connect your database** (Supabase project URL + anon key + service-role key) and **2 · Create your admin account** (email + password) — plus a single collapsed **"Optional: payments, email, maps and AI"** disclosure (all four default to "Skip for now") and one **"Finish setup"** button. Removed the 5-step stepper, the storage-provider matrix (Supabase-only — it is the canonical store), the per-field env/copy noise, the Cloudflare panel, and the rich schema-fix card (schema errors now render as plain-text numbered steps in the inline error box). A compact production-only banner still lists the exact `npx wrangler secret put …` commands for any missing Supabase secret (from `dataStoreSummary()`), with a "Check again" reload.
  - **🔓 Email, maps and the AI engine are now OPTIONAL (skippable).** `normalizePlatformSettingsInput` (`services/config/platform-settings.ts`) accepts a missing/blank/`'none'` provider for `mail`, `map` and `ai` (payments were already optional), so a fresh store boots from **just Supabase + an admin account**. The driver factories already fell back gracefully: `EmailFactory` → `null` (skip sends), `MapFactory` → OpenStreetMap (no key), `AiFactory` → `null` (CSS/SVG animation presets). `PlatformSettingsInput` (`services/config/types.ts`) widened `mail_provider` / `map_provider` / `ai_provider` to `| null`. The SQL `global_platform_settings` columns were already nullable — **no migration needed**.
  - **🧪 Tests:** `tests/setup-normalize.test.ts` rewrote "AI is mandatory" into "AI / Email / Maps are optional" (3 cases, +2 net). `npm test` **267/267**, `tsc --noEmit` clean, `eslint` 0/0 on touched files, `npm run build` compiles every route incl. `/admin/setup`. No Redis keys added or changed.
- **2026-08-26 — Stupid-proof Cloudflare data-store gate + storage redundancy (`supabase-cloudflare-hard-gate`):**
  - **🔒 The 3 Supabase secrets are now a HARD gate in production.** `/api/admin/setup` POST rejects with `422 code: 'cloudflare_supabase_required'` until `SUPABASE_URL` + `SUPABASE_ANON_KEY` + `SUPABASE_SERVICE_ROLE_KEY` are present as **REAL environment variables** (not the wizard's volatile inline override) — because inline credentials are lost on every cold start and would lock the buyer out after a redeploy. Local dev is exempt. This closes the mismatch where Cloudflare showed only the plaintext `vars` (which never include secrets) while the wizard asked for secrets that live in the "Secrets" column.
  - **🧭 Setup step 1 rebuilt as a stupid-proof "Connect your database" gate** (`app/admin/setup/page.tsx`): new `SupabaseGate` (live ✓/✗ badge, a numbered "do these 3 things in order" walkthrough with copy-paste `npx wrangler secret put` commands, and a "⟳ Check again" re-scan button) + `RedundancyGate` (optional Upstash safety mirror) + reusable `HelpToggle` (collapsible "how do I do this?" drops) + `HelpLink` (external links to supabase.com / upstash.com / dash.cloudflare.com). The Continue button is disabled with "🔒 Connect Supabase to continue" in production until the gate passes; `validateStep(0)` + `handleNext` double-gate it client-side.
  - **🧹 `wrangler.jsonc` `vars` block cleaned up** — the 6 confusing empty plaintext placeholders (`ADMIN_VERIFY_EMAIL`, `BRAND_NAME`, `SUPPORT_EMAIL`, `MAINTENANCE_MODE`, `RESEND_FROM`, `LICENSE_ENFORCED`) were removed so the dashboard no longer shows meaningless empty "Variables"; only `STORAGE_PROVIDER` remains (a real, non-secret default). The secrets reference now documents `STORAGE_REPLICAS`.
  - **🛡 Storage write-through redundancy** — new `lib/storage/replicated.ts` `ReplicatedStorageClient` (writes mirror to every replica, primary is canonical; reads fail over to replicas on throw/empty). New env `STORAGE_REPLICAS` (comma-separated, e.g. `upstash`) + `resolveReplicaProviders()`/`normalizeStorageProvider()` in `lib/storage/types.ts`, wired into `createStorageClient()` in `lib/storage/index.ts` (also exported `activeReplicaProviders()`). This protects against a single vendor losing years of customer data.
  - **🔍 `lib/env-discovery.ts`** gained `dataStoreSummary()` (per-store reachability + exact missing creds with copy commands) and `supabaseEnvFullySet()`; `/api/admin/setup` GET now returns `dataStores`, `supabaseEnvReady` and `isProduction`.
  - **🧪 Verified:** `npm test` **265/265** (new `tests/storage-replicated.test.ts` 5 cases + `tests/storage-provider.test.ts` +3 + `tests/env-discovery.test.ts` +2), `tsc --noEmit` clean, `eslint` 0/0, `wrangler.jsonc` re-parses as valid JSONC. No Redis keys added or changed (`STORAGE_REPLICAS` is an env var, not a key).
...
- **2026-09-05 — Non-surface refactor of inventory-sync: three-way DOM suppression + payload auto-commit (`inventory-sync-deep-refactor`):**
  - **🧱 DOM suppression is now a THREE-way gate, fully unmounting (not disabling) the variant grid.** New `isSyncEnabled = Boolean(cat.syncWithExisting || cat.inventorySyncSlug || String(cat._syncDraft ?? '').trim())` in Tab 2 (`app/admin/page.tsx`). The entire variant input grid (variant name, SKU, price, units, Stripe ID, limits, checkout mode, winners) is wrapped in `{!isSyncEnabled && (<>…</>)}` — so it is COMPLETELY OMITTED from the DOM the moment the "Sync with existing slug?" box is checked, a slug is committed, OR a draft is being typed. Every `disabled={synced}` and `opacity: synced ? 0.6 : 1` attribute was removed (10 inputs + the checkout-mode `<select>`, which now only disables on a product-level FCFS lock). `synced` still only selects the badge card vs the slug prompt. The result: only the slug prompt (unlinked) or the "🔗 Synced with [SLUG]" badge card + Unlink renders, and no dead disabled inputs or "Limits (0 = Unlimited)" labels can leak into a synced row.
  - **📦 Payload builder auto-commits uncommitted sync intent.** `saveProduct`'s `priceCategories` mapping now promotes a typed draft to a committed slug before serialization: if `c.syncWithExisting` or a non-empty `c._syncDraft` is present, `out.inventorySyncSlug = slugifyName(draft || existing)`. Every variant now explicitly sends `inventorySyncSlug` AND `inventoryPoolId` as a real slug or `null` (never absent), so a reorder/status save can never silently un-link a pool.
  - **🔧 Backend verified (no functional change needed) + self-documenting note.** `app/api/admin/products/route.ts` spreads `{ ...(c || {}) }` so `inventorySyncSlug`/`inventoryPoolId` pass through unchanged to `bindInventoryPoolToCategories` (which drops `null`/blank and binds the canonical id); `validatePriceCategories` is read-only (price/size only, never strips the pair); `saveProduct` persists the whole object via `JSON.stringify` into `store:products`; `normalizePriceCategory`/`loadProducts` re-hydrate both fields on reload. Added an explicit comment so the retention is never accidentally whitelisted away.
  - **🧪 Verified:** `tsc --noEmit` clean, `npm test` **345/345**. No new Redis keys. (The same two pre-existing `app/admin/page.tsx` lint notices at line 104 and the Checkout Mode tab apostrophe remain untouched.)

- **2026-09-05 — Unified variant-mode dropdown + slug-sync hydration fix + commerce-rule accordion removal (`unify-variant-mode-slug-sync-hydration`):**
  - **🔗 Slug-sync state hydration (root cause of saved-slug loss).** `editProduct` now hydrates every loaded variant's transient editor state via a new `hydrateCategorySyncState()` helper — `syncWithExisting = Boolean(committedSlug)` and `_syncDraft = committedSlug` (from `inventorySyncSlug || inventoryPoolId`) — so a saved synced variant is recognized on FIRST render (`isSyncEnabled` = true) and the standard input grid is suppressed, not merely the "🔗 Synced" badge card. `saveProduct`'s payload mapping now resolves the final slug from (draft → committed) and NEVER wipes `inventorySyncSlug` to null when `syncWithExisting` is enabled (it only un-links when there is genuinely no sync intent).
  - **🎛 Removed the "⚙️ Variant Rules & Commerce Config" accordion** (and the now-unused `CommerceJsonField` JSON-textarea editor) — the 10-primitive selector + raw JSON blocks are gone from the variant row.
  - **📋 Unified "Variant Mode" dropdown.** Each variant row now has ONE primary dropdown with 8 human-friendly modes (⚡ Instant Buy (FCFS) / 🎟 Raffle / Allocation Draw / 📦 Preorder / ⏰ Time Slot / 🔒 Gated Access / 👥 Group Buy / 📉 Dutch Auction / 💬 RFQ / Quote) via a new `VARIANT_MODE_OPTIONS` list + `setVariantCommerceMode()` (stores `commerceMode` AND derives the legacy `checkoutMode` — RAFFLE for allocation draws, FCFS otherwise). `effectiveSizeCheckoutMode` now resolves the universal commerce mode first; a new `variantModeValue()` derives the dropdown's selected value from `commerceMode` → legacy `checkoutMode` so legacy RAFFLE/FCFS variants still show their real mode.
  - **🔀 Conditional commerce inputs** render only for the selected mode: Raffle → "Draw date / time" (`scheduleConfig.drawAt`) + "Winner count" (moved here from the old row, keeping the `pf-winnerstiers-${idx}` id so `focusIssue`/red-border highlighting still work); Preorder → "Estimated ship date" (`scheduleConfig.estimatedShipDate`) + "Deposit %" (`billingRule.depositPercent`); Gated Access → "Access key / tag" (`accessRule.accessCode`). Instant Buy shows no extra inputs. `saveProduct` compiles `commerceMode` / `accessRule` / `billingRule` / `scheduleConfig` into clean JSON objects (dropping empty blocks) before saving.
  - **🧪 Verified:** `tsc --noEmit` clean, `npm test` **345/345**, `eslint app/admin/page.tsx` **0 errors** (also fixed the pre-existing unescaped apostrophe in the Checkout Mode tab). No new Redis keys — `commerceMode` + the three blocks live on the variant object inside `store:products`, already preserved by `normalizePriceCategory`/`loadProducts`.

- **2026-09-05 — Inventory-sync save flow hardening: sync-slug guarantees + loud error alerts + payload telemetry (`inventory-sync-save-hardening`):**
  - **🔗 Synced-variant payload guarantee.** In `saveProduct`'s `priceCategories` mapping (`app/admin/page.tsx`), a variant linked into a shared-inventory pool now ALWAYS carries a numeric price + string SKU: after the source-category backfill, `out.price = out.price || 0` and `out.sku = out.sku || ''` run unconditionally inside the `if (poolSlug)` branch. Because the synced variant's input grid is fully unmounted (no form fields), an empty price/SKU can never trip the backend JSON-schema / price validator — a synced variant is exempt from the `empty_price` sanity error AND the server-side `validatePriceCategories` gate (which already filters `inventorySyncSlug` rows).
  - **📢 Loud, unmissable save failures.** The `adminFetch('/api/admin/products')` call's error paths now surface via `window.alert` instead of failing silently: non-200 responses alert `'API Error (' + res.status + '): ' + errorText` (resolving `error` → `blocking[]` → raw text), and a thrown `fetch()` (network/CORS/abort) alerts `'Network Error: ' + err.message`. Pre-flight validation already alerts `'Save prevented: …'` for name/slug/variant/sanity blockers — no save path now fails without visible feedback.
  - **📡 Payload telemetry.** Immediately before the POST, the client logs `[CLIENT SAVE PAYLOAD]` with `priceCategories` reduced to `{ size, slug: inventorySyncSlug }` — so an operator (or support agent) can verify exactly which sync slugs were sent.
  - **🔒 Backend verified (no functional change needed).** `app/api/admin/products/route.ts` already spreads `{ ...(c || {}) }` so `inventorySyncSlug`/`inventoryPoolId` pass through to `bindInventoryPoolToCategories` (write-time bind) before `saveProduct` persists the whole object via `JSON.stringify`; `lib/server-config.ts` `normalizePriceCategory` (lines ~940-944) re-hydrates both fields so `loadProducts` returns the link on page refresh, and the price gate filters synced rows. `lib/product-sanity.ts` exempts synced variants from `empty_price`.
  - **🧪 Verified:** `npx tsc --noEmit` clean, `npm test` **345/345**, `eslint app/admin/page.tsx` **0 errors**. No new Redis keys — the guarantees live entirely in the client payload builder; `inventorySyncSlug`/`inventoryPoolId` remain the only persisted fields.
