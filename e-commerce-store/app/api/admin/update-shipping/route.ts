  ...
  try {
    ...
    for (let i = 0; i < all.length; i++) {
      ...
      if (
        ...
        // Issue the post-delivery trial credit once per order. Per-sampler
        // overrides (credit value, min order, expiry, prefix, eligibility) win;
        // product-level deliveryIncentive* values are the fallback.
        if (shippingStatus === 'DELIVERED') {
          try {
            const product = Object.values(liveProducts).find((p: any) => p.name === variant || p.id === e.productId) as any;
            const sampler = resolveSamplerConfig(product, String(e.size || size));
