...
const DEFAULT_PRODUCTS = [
  ...
  },
  {
    ...
    isActive: true, isArchived: false, isUpcoming: false, isRaffle: true, checkoutMode: 'RAFFLE', productType: 'raffle', sortOrder: 2,
    notes: [
      { label: 'MODE', name: 'How it works', text: 'Mixed formats. The Sampler Set ($19) is first-come, first-served — pay now and it ships. The Full Bottle ($145) runs a raffle: secure your entry with a card and winners are charged when the draw fires. Buying the Sampler issues a $15 credit toward the Full Bottle after delivery.' },
      { label: 'WHY', name: 'Why this drop matters', text: 'This product demos the mixed-format engine: per-size checkout modes, the cart flow, multi-size pricing (Sampler vs Full Bottle), and the per-size trial incentive.' },
      { label: 'PROS', name: 'Pros', text: 'Low-friction sampler converts browsers instantly while the full size stays scarce and selective — best of both worlds.' },
      { label: 'CONS', name: 'Cons / tradeoffs', text: 'Two payment flows on one page means the product needs clear size-level labels, which this page provides.' },
      { label: 'PURPOSE', name: 'Best used for', text: 'Any drop where a cheap trial SKU should convert immediately while the hero size stays a raffle.' },
    ],
  ...
  },
  {
    ...
    isActive: true, isArchived: false, isUpcoming: false, isRaffle: true, checkoutMode: 'RAFFLE', productType: 'raffle', sortOrder: 13,
    notes: [
      { label: 'MODE', name: 'How it works', text: 'Mixed formats. The Discovery Kit ($42) is first-come, first-served; the Full Bottle and Grand Size are raffles. Buying the kit triggers a $20 store credit toward the full bottle after delivery, and up to 4 units per cart.' },
      { label: 'WHY', name: 'Why this drop matters', text: 'The most complete mixed-format demo: per-size raffle/FCFS modes, multi-SKU pricing, quantity caps, and an incentive that upsells a small first order into the hero size.' },
      { label: 'PROS', name: 'Pros', text: 'High average order value, natural repeat-buy behavior, and a built-in demo of delivery incentives plus mixed checkout.' },
      { label: 'CONS', name: 'Cons / tradeoffs', text: 'More moving parts than a simple drop — needs clean product config to avoid confusion.' },
      { label: 'PURPOSE', name: 'Best used for', text: 'Bundles, memberships, and any drop where the goal is maximizing order value while keeping hero sizes scarce.' },
    ],
