...
export async function GET() {
  const items: EnvStatusItem[] = [
    {
      ...
      buildTime: false,
      sensitive: false,
      hint: 'UPSTASH_REDIS_REST_URL + UPSTASH_REDIS_REST_TOKEN (or the Vercel KV auto-injected KV_REST_API_URL / KV_REST_API_TOKEN pair).',
    },
    {
      key: 'Redis token',
      label: 'Redis access token',
      required: true,
      set: has('UPSTASH_REDIS_REST_TOKEN', 'KV_REST_API_TOKEN', 'REDIS_TOKEN'),
      aliases: ['UPSTASH_REDIS_REST_TOKEN', 'KV_REST_API_TOKEN', 'REDIS_TOKEN'],
      buildTime: false,
      sensitive: true,
    ...
    },
    {
      ...
      buildTime: false,
      sensitive: true,
      hint: 'Secret — set in the platform (Vercel). The storefront will not charge without it.',
    },
    {
  ...
  items.push(
    ...
    },
    {
      ...
      buildTime: false,
      sensitive: true,
      hint: 'Secret — guards /api/cron/*. Required for scheduled/auto draws.',
    },
    {
      key: 'Resend',
    ...
    },
    {
      ...
      buildTime: true,
      sensitive: false,
      hint: 'Used in emails, OG/social cards and canonical links. NEXT_PUBLIC_* values are baked at build time — set in the same environment and redeploy.',
    },
    {
      key: 'Brand name',
