  ...
  try {
    ...
    const result = await withRedisLock(redis, `redeem-points:${sessionUser.email}`, async () => {
      ...
      if (maxRedeemPoints > 0 && requestedPoints > maxRedeemPoints) {
        return { error: `Maximum redemption is ${maxRedeemPoints.toLocaleString()} points per transaction.`, status: 400 } as const;
      }
    ...
    if ('error' in result.value) {
      return NextResponse.json({ error: result.value.error }, { status: result.value.status });
    }
