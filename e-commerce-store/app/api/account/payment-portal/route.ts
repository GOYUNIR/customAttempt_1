...
export async function POST(request: Request) {
  try {
    ...
    if (!email || last4.length !== 4) return NextResponse.json({ error: 'Enter your email and card digits first.' }, { status: 400 });
    ...
    const portalSession = await stripe.billingPortal.sessions.create({ customer: customerId, return_url: `${protocol}://${hostHeader}/account` });
