...
export async function POST(request: Request) {
  try {
    ...
    await archiveEntry(redis, { email, variant, size, shippingAddress: target.parsed.shippingAddress || target.parsed.address || 'Unknown', id: target.parsed.customerId || 'n/a', registeredAt: new Date().toISOString(), type: 'CANCELLED_BY_USER' });
