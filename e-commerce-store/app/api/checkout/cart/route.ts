...
export async function POST(request: Request) {
  ...
  // keeps the path below, unchanged; any other store's cart is checked out on
  // its OWN Stripe account (lib/tenant-checkout.ts, phase 3).
  ...
  if (!who.isDefault) {
    try {
      ...
      return (await startTenantCartCheckout({ tenantId: who.tenantId, tenantSlug: who.slug, origin: requestOriginOf(request), body })) as any;
    } catch (err: any) {
      console.error('[checkout/cart] tenant ' + who.tenantId + ' failed', err?.raw?.message || err?.message || err);
