...
export async function POST(request: Request) {
  try {
    ...
    if (!email || last4.length !== 4) {
      return NextResponse.json({ error: 'Enter your email and the last 4 digits of your card.' }, { status: 400 });
    }
    ...
    if (verified.length === 0) {
      return NextResponse.json({ error: 'No matching entry found. Double check your email and card digits.' }, { status: 404 });
    }

    const entries = verified.map((m) => ({
      variant: m.variant,
      size: m.size,
