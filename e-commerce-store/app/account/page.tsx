  ...
  const saveAddress = async (entry: EntryRecord) => {
    ...
    try {
      const res = await fetch('/api/account/update-address', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, last4, variant: entry.variant, size: entry.size, newAddress: addressDraft }),
      });
      const data = await res.json();
      if (res.ok) {
    ...
    <main style={{ minHeight: '100vh', background: configPalette.primaryBackground, color: configPalette.textMain, padding: '80px 20px 60px', boxSizing: 'border-box' }}>
      <div style={{ maxWidth: '420px', margin: '0 auto' }}>
        ...
        <p style={{ fontSize: '12px', color: configPalette.textMuted, margin: '0 0 24px 0' }}>
          Verify with the email and the last 4 digits of the card you used to enter.
        </p>
        ...
        <div style={{ background: configPalette.cardBackground, border: `1px solid ${configPalette.cardBorder}`, borderRadius: '20px', padding: '20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <input
            ...
            style={{ width: '100%', padding: '14px', borderRadius: '12px', background: '#16161a', border: `1px solid ${configPalette.cardBorder}`, color: '#fff', fontSize: '13px', boxSizing: 'border-box' }}
          />
          <input
            type="text"
            inputMode="numeric"
          ...
          <button
            ...
            style={{ width: '100%', padding: '14px', borderRadius: '30px', background: configPalette.checkoutCtaButton, color: '#fff', border: 'none', fontWeight: 'bold', fontSize: '13px', cursor: isBusy ? 'not-allowed' : 'pointer' }}
          >
        ...
        {message && (
          <p style={{ marginTop: '16px', fontSize: '12px', textAlign: 'center', color: '#cbd5e1' }}>{message}</p>
        )}
          ...
          <div style={{ marginTop: '24px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {entries.map((entry) => (
              <div key={`${entry.variant}-${entry.size}`} style={{ background: configPalette.cardBackground, border: `1px solid ${configPalette.cardBorder}`, borderRadius: '16px', padding: '16px' }}>
                ...
                <div style={{ fontSize: '11px', color: configPalette.textMuted, marginTop: '4px' }}>
                  Shipping to: {entry.shippingAddress}
                </div>
                ...
                {editingAddressFor === `${entry.variant}-${entry.size}` ? (
                  <div style={{ marginTop: '10px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <input
                      ...
                      style={{ width: '100%', padding: '10px', borderRadius: '10px', background: '#16161a', border: `1px solid ${configPalette.cardBorder}`, color: '#fff', fontSize: '12px', boxSizing: 'border-box' }}
                    />
                  ...
                  <div style={{ display: 'flex', gap: '8px', marginTop: '12px', flexWrap: 'wrap' }}>
                    <button
                      onClick={() => { setEditingAddressFor(`${entry.variant}-${entry.size}`); setAddressDraft(entry.shippingAddress); }}
                      style={{ padding: '8px 12px', borderRadius: '10px', border: `1px solid ${configPalette.cardBorder}`, background: 'transparent', color: '#ccc', fontSize: '11px', cursor: 'pointer' }}
                    >
                      Edit Address
                    </button>
                    <button
                      ...
                      disabled={isBusy}
                      style={{ padding: '8px 12px', borderRadius: '10px', border: '1px solid #ff3b30', background: 'transparent', color: '#ff3b30', fontSize: '11px', cursor: 'pointer' }}
                    >
                      Cancel Entry
                    </button>
                  </div>
            ...
            <button
              ...
              disabled={isBusy}
              style={{ width: '100%', padding: '14px', borderRadius: '30px', background: 'transparent', border: `1px solid ${configPalette.cardBorder}`, color: configPalette.textMain, fontSize: '13px', fontWeight: 'bold', cursor: 'pointer', marginTop: '8px' }}
            >
