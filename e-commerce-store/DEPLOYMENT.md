...
Apply `supabase/migrations/00001` → `00016` in order (Supabase CLI: `supabase db push`, or paste each file into the SQL editor in order). One-line purpose of each:
...
- **Merchant Hub is not a separate app.** `app.site.com` gets its own required ROLE (§3.2) but still renders the same `app/admin` route tree as `admin.site.com` — a genuinely separate merchant-control-center app (distinct nav, distinct surface area) is future work.
- **Comparative competitive claims are unverified** (see "Architecture positioning" above) — no benchmarking infrastructure exists in this environment.
