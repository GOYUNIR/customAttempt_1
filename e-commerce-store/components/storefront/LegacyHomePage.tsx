...
/**
 * LEGACY HOME PAGE — the original hardcoded homepage (hero shaders, AI
 * animation, priority-drops grid), relocated verbatim from `app/page.tsx`
 * when the theme-customizer system (Phase 5) was added. Deliberately NOT
 * rewritten — this is the same deeply-coupled, untestable-by-visual-tool
 * logic every other "don't blind-rewrite" carve-out this session applies
 * to. `app/page.tsx` now renders this when a tenant has no active theme
 * row (the default, today, for every existing deployment) and
 * `components/storefront/ThemeSections.tsx` when one exists.
 */
