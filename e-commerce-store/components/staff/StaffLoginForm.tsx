...
import { STAFF_REALMS, type StaffRealmKey } from '@/lib/staff-realms';

/**
 * THE staff sign-in form — one component, three realms.
 *
 * Rendered by /admin/login, /app/login and /sales/login. Only the copy and the
 * post-sign-in destination differ; the credential flow is identical and lives
 * here once. See lib/staff-realms.ts for why the routes are separate but the
 * mechanism is not.
 *
 * Every realm POSTs to `/api/admin/login`. That endpoint is the shared staff
 * auth backend — it is not "the admin one" that sales borrows. Its path is
 * historical, and renaming it would touch the CSRF exemptions, the Edge path
 * fence and the Basic-Auth fallback all at once; that is a separate change
 * from fixing a portal that cannot be signed into.
 */
...
export default function StaffLoginForm({ realm: realmKey }: { realm: StaffRealmKey }) {
  ...
  async function submit(e: FormEvent) {
    ...
    if (!email.trim() || !password) {
      setError(`Enter your ${realm.audience} email and password.`);
      return;
    }
    setBusy(true);
    try {
